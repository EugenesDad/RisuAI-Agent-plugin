...
(async () => {
  // ============================================================================
  // 1. 상수 및 기본값 정의 (Constants & Defaults)
  // ============================================================================
...
6. Match key names and value types exactly.`,
  ...
  // ============================================================================
  // 2. 전역 상태 및 유틸리티 (Global State & Utilities)
  // ============================================================================
  const uiIds = [];
  let replacerFn = null;
  ...
  // ============================================================================
  // 3. 벡터 데이터베이스 및 캐시 (pluginStorage Vector Cache)
  // Risuai.pluginStorage를 IndexedDB 대신 사용 (iframe 샌드박스 내에서 IDB 접근 불가)
  // 「index + per-card」분할 전략 채택:
  ...
  async function saveEmbeddingCacheStore(storeToSave = null) {
    ...
    try {
      ...
      try { await Risuai.log(`${LOG} 경고: 벡터 캐시 저장 실패 (${err.message})`); } catch { }
    }
  }
  ...
  async function clearAllEmbeddingCache() {
    ...
    // 2. pluginStorage 벡터 인덱스 및 모든 카드 데이터 초기화
    try {
      const idxRaw = await Risuai.pluginStorage.getItem(VCACHE_INDEX_KEY);
      if (idxRaw) {
        const idx = JSON.parse(idxRaw);
  ...
  function parsePossiblyWrappedJson(text) {
    ...
    // 2. Fallback 혼합 전략:
    //    먼저 lastBrace 사용(문자열 값에 }가 포함된 경우 정확히 처리),
    //    파싱 실패시 depth 추적으로 전환(모델 출력이 여러 JSON 객체를 나란히 출력하는 경우 정확히 처리).
  ...
  function resolveExtractorConfig() {
    ...
    // provider_key_map에서 공급자별로 독립 저장된 키 읽기
  ...
  // ============================================================================
  // 5. 모델 인증 및 외부 API 관리 (Model API & Headers)
  // ============================================================================
  ...
  // ============================================================================
  // 6. 핵심 텍스트 및 벡터 처리 로직 (Text & Vector Algorithms)
  // ============================================================================
  function getEmbeddingTokenLimit(requestModel) {
    // 입력별 토큰 제한; API는 이 길이를 초과하는 입력을 잘라냄.
  ...
  function getEmbeddingBatchTokenLimit(requestModel) {
    // API 요청당 허용되는 최대 총 토큰 수 (모든 입력의 합계).
    const m = String(requestModel || "").toLowerCase();
  ...
  function splitIntoParagraphChunks(content, maxChars = Infinity) {
    ...
    for (let level = 1; level <= 4; level++) {
      ...
      if (level === 1) {
        ...
        // L1 == 0 또는 == 1: 더 이상 깊이 내려가지 않음 — 항목이 단일 주제 단위임
        break;
      } else {
        // 레벨 N>1: 부모 레벨에도 >= 2 섹션이 있을 때만 분할
  ...
  async function fetchEmbeddingVectorsRemote(texts, cfg) {
    const runner = async () => {
      ...
      const allVectors = [];
      await Risuai.log(`${LOG} [Embed] 시작: provider=${cfg.provider}, model=${cfg.requestModel}, 총 ${input.length}개 → ${batches.length}배치`);
      ...
      for (let bi = 0; bi < batches.length; bi++) {
        ...
        try {
          if (cfg.format === "google") {
            ...
            if (!baseUrl || !model) throw new Error("Google Embedding: URL 또는 model이 설정되지 않았습니다");
            ...
            await Risuai.log(`${LOG} [Embed] Google 응답 via=${via}: status=${res.status}, ok=${res.ok}`);
            ...
            await Risuai.log(`${LOG} [Embed] Google 파싱: 벡터 ${batchVectors.length}개 수신`);
          } else {
            const url = normalizeEmbeddingUrl(cfg.url, cfg.format);
            const model = safeTrim(cfg.requestModel || cfg.selectedModel || "");
            if (!url || !model) throw new Error(`Embedding: URL(${url}) 또는 model(${model})이 설정되지 않았습니다`);
            ...
            await Risuai.log(`${LOG} [Embed] OpenAI-Compat 응답 via=${via}: status=${res.status}, ok=${res.ok}`);
            if (!isResponseLike(res) || !res.ok) {
              const errTxt = await readResponseErrorText(res);
            ...
            await Risuai.log(`${LOG} [Embed] OpenAI-Compat 파싱: 벡터 ${batchVectors.length}개 수신`);
          }
          if (batchVectors.length !== batchInput.length || batchVectors.some((v) => !Array.isArray(v) || !v.length)) {
            throw new Error(`${batchLabel} 벡터 수 불일치 (got ${batchVectors.length}, expected ${batchInput.length})`);
          }
          allVectors.push(...batchVectors);
        } catch (e) {
          console.error(`${LOG} Embedding ${batchLabel} failed:`, e);
          await Risuai.log(`${LOG} [Embed] ❌ ${batchLabel} 실패: ${e?.message || String(e)}`);
          for (let j = 0; j < batchInput.length; j++) allVectors.push([]);
          break;
        }
      }
      while (allVectors.length < input.length) allVectors.push([]);
      await Risuai.log(`${LOG} [Embed] 완료: 총 ${allVectors.filter(v => v && v.length).length}/${input.length}개 성공`);
      return allVectors;
    };
  ...
  async function getEmbeddingsForTexts(texts, skipCache = false) {
    ...
    if (out.some((v) => !Array.isArray(v) || !v.length)) throw new Error("일부 임베딩 벡터 가져오기 실패.");
    return out;
  }
  ...
  async function getLorebookContextByVector(names, conversationMessages) {
    ...
    try {
      ...
      try { await Risuai.log(`${LOG} 임베딩 벡터 검색 fallback: ${e?.message || String(e)}`); } catch { }
  ...
  function limitConversationByRounds(baseMessages, rounds) {
    ...
    // r=0은 '제한 없음, 전부 읽기'를 의미
    if (r === 0) return baseMessages || [];
    return (baseMessages || []).slice(-Math.max(1, r * 2));
  }

  // ============================================================================
  // 7. 모델 추출 및 지식 주입 (Extractor & Knowledge Injection)
  // ============================================================================
  ...
  function normalizeOutputEntry(entry, target) {
    ...
    // retention_enabled: write_mode=append일 때만 유효, 활성화 시 오래된 턴 데이터를 자동으로 정리
    // retention_after: 이 턴 수를 초과하면 정리 시작 (0 = 즉시)
    // retention_keep: 정리 후 유지할 최신 턴 블록 수
    ...
    const retentionKeep = Math.max(0, toInt(entry?.retention_keep, 5));  // 0 = 전체 삭제
    return {
  ...
  function normalizeModelCall(call, index = 0) {
    ...
    const nameFallback = index === 0 ? "호출 메모A" : index === 1 ? "호출 메모B" : `호출 메모${index + 1}`;
    const every = Math.max(1, toInt(call?.every_n_turns, 1));
    const readRounds = Math.max(0, toInt(call?.read_dialogue_rounds, 4));  // 0 = 전부 읽기
    const entries = parseOutputEntries(call?.entries, target);
    // allow_parallel: 이 호출을 다른 allow_parallel 호출과 동시에 발행 허용 (최대 3개 병렬)
  ...
  async function batchUpsertLocalLore(writes, roundIndex = 0) {
    ...
    return mutexLoreWrite.run(async () => {
      try {
        ...
        for (const { loreName, writeMode, alwaysActive, content } of writes) {
          ...
          if (writeMode === "overwrite") {
            ...
            if (existingIndex < 0) {
              ...
              // 턴 레벨 중복 제거: 동일한 턴 블록이 이미 존재하면 건너뜀
        ...
        await Risuai.log(`${LOG} batchUpsertLocalLore 실패: ${err?.message || String(err)}`);
        return false;
      }
    });
  }
  ...
  async function applyRetentionCleanup(userMsgCount) {
    ...
    return mutexLoreWrite.run(async () => {
      try {
        ...
        console.error(`${LOG} applyRetentionCleanup 실패:`, err);
      }
    });
  }
  ...
  async function writeOutputsForCall(modelCall, raw, parsed, reqHash, roundIndex = 0) {
    ...
    if (entries.length > 1 && (!parsed || typeof parsed !== "object")) {
      // 멀티 항목 호출은 유효한 JSON 객체를 반환해야 함. 데이터를 무음으로 건너뛰지 않고
      // 호출자가 메인 모델을 중단할 수 있도록 하드 실패로 처리.
      throw new Error(`보조 모델 (${modelCall.name})이 JSON 객체를 반환해야 하지만 파싱에 실패했습니다. raw 앞 100자: ${String(raw || "").slice(0, 100)}`);
    }
    ...
    if (pendingWrites.length === 0) {
      if (entries.length > 0) {
        await Risuai.log(`${LOG} 경고: 모델 반환 내용이 예상 항목 이름 (${entries.map(e => e.lorebook_name).join(", ")})과 일치하지 않거나 형식 오류로 쓰기를 건너뜁니다.`);
      }
      return;
    }
    ...
    if (!wrote) {
      await Risuai.log(`${LOG} 경고: 로컬 로어북에 데이터 일괄 쓰기 시 저장 오류 발생 (call: ${modelCall.name}).`);
    }
  }
  ...
  async function callOpenAICompat({ url, apiKey, model, messages, timeoutMs, temperature }) {
    ...
    if (!content || typeof content !== "string") throw new Error("Extractor가 빈 내용을 반환했습니다.");
  ...
  async function callGoogleGenerative({ url, apiKey, model, messages, timeoutMs, temperature }) {
    ...
    if (!finalUrl) throw new Error("Google URL/model이 없습니다.");
    const headers = { "Content-Type": "application/json" };
    ...
    if (!content) throw new Error("Google extractor가 빈 내용을 반환했습니다.");
  ...
  async function callClaudeCompat({ url, apiKey, model, messages, timeoutMs, temperature }) {
    ...
    if (!finalUrl || !safeTrim(model)) throw new Error("Claude URL/model이 없습니다.");
    ...
    if (!content) throw new Error("Claude extractor가 빈 내용을 반환했습니다.");
    return { parsed: parsePossiblyWrappedJson(content), raw: content };
  }
  ...
  async function callExtractorStrict({ url, apiKey, model, messages, timeoutMs, mode, format = "openai", temperature = 0 }) {
    const runner = async () => {
      ...
      if (!safeTrim(result?.raw)) throw new Error(`Extractor ${mode}가 빈 내용을 반환했습니다.`);
    ...
    if (configCache.vector_search_enabled === 0) {
      ...
      if (staticRaw) {
        try {
          ...
          await Risuai.log(`${LOG} ⚠️ static_knowledge_chunks 파싱 불가, 즉시 가져오기 모드로 전환`);
        }
      } else {
      }
      ...
      if (allChunks.length === 0) {
        ...
        await Risuai.log(`${LOG} ⚠️ 사용 가능한 청크 없음 (static_knowledge_chunks가 비어있고 dynamicChunks도 없음), 지식 베이스 주입 건너뜀. 초기화(Step 0)가 완료되었는지 확인하세요.`);
      }
      ...
      try {
        ...
        if (missingTexts.length > 0) {
          ...
          for (let i = 0; i < missingTexts.length; i += batchSize) {
            ...
            await Risuai.log(`${LOG} Agent: 벡터 연산 실행 중 (${i + 1}~${Math.min(i + batchSize, missingTexts.length)}/${missingTexts.length})...`);
        ...
        await Risuai.log(`${LOG} 벡터 검색 실패: ${e.message}, 토큰 매칭으로 전환.`);
      }
    }
  ...
  async function mergeToSystemPromptWithRewrite(messages, payload, queryText) {
    ...
    try {
      ...
      throw new Error(`지식 베이스 주입 실패 (Knowledge Injection Failed): ${e.message}`);
    }
  ...
  async function runStep0Classification(char, resolvedGlobalNote, staticDataHash, resumeMode = false) {
    ...
    lorebook.forEach((l, idx) => {
      ...
      // RisuAI 내부에서는 alwaysActive, SillyTavern 호환용으로 constant 필드가 '항상 활성화'를 나타냄
    ...
    if (resumeMode) {
      ...
      } catch { /* resumeMode 읽기 실패, 전체 재실행으로 fall through */ }
    }
    ...
    if (skipClassification || chunksToClassify.length === 0) {
      ...
      for (let i = 0; i < chunksToClassify.length; i += batchSize) {
        ...
        await Risuai.log(`${LOG} 초기 데이터 분류 중 (${currentBatch}/${totalBatches})...`);
  ...
  // ============================================================================
  // 8. 인터페이스 및 옵션 렌더링 (UI & Options Rendering)
  // ============================================================================
...
.pse-call-card[data-call-parity="odd"] { border-left-color:var(--pse-accent-green); background:linear-gradient(180deg, rgba(5, 150, 105, 0.12) 0%, var(--pse-input-bg) 100%); }
  ...
  async function abortMainModelWithAuxError(message) {
    const msg = String(message || "보조 모델 호출 또는 처리 실패");
    await Risuai.pluginStorage.setItem("last_lore_sync_error", msg);
    await Risuai.log(`${LOG} ❌ 오류: ${msg}. 메인 모델 응답을 강제 중단했습니다!`);
    // 캐시된 해시를 지워 다음 재시도 시 오래된 캐시가 아닌 추출이 재실행되도록 함
    try { await Risuai.safeLocalStorage.removeItem("last_req_hash"); } catch { }
    throw new Error(`[RisuAI Agent 오류] ${msg} \n(크레딧 절약을 위해 메인 모델 요청을 차단했습니다)`);
  }
  ...
  async function renderSettingsUI() {
        ...
        <div class="pse-card">
          ...
          <div class="pse-page active" data-page="1">
            <div class="pse-section pse-model-section-a">
              ...
              <label class="pse-label">온도 (Temperature)</label>
              ...
              <label class="pse-label">요청 모드 (Concurrency)</label>
              <select id="extractor_a_concurrency" class="pse-input">
                <option value="1" ${Number(configCache.extractor_a_concurrency) === 1 ? "selected" : ""}>병렬 (복수 요청 동시 발송 허용)</option>
                <option value="0" ${Number(configCache.extractor_a_concurrency) === 0 ? "selected" : ""}>순차 (단일 스레드 요청만 가능)</option>
              </select>
            </div>

            <div class="pse-section pse-model-section-b">
              <div class="pse-section-title">보조 모델 (Auxiliary Model)</div>
              <label class="pse-label">제공자</label>
              ...
              <select id="extractor_b_concurrency" class="pse-input">
                <option value="1" ${Number(configCache.extractor_b_concurrency) === 1 ? "selected" : ""}>병렬 (복수 요청 동시 발송 허용)</option>
                <option value="0" ${Number(configCache.extractor_b_concurrency) === 0 ? "selected" : ""}>순차 (단일 스레드 요청만 가능)</option>
              </select>
            </div>
            ...
            <div class="pse-section pse-model-section-embed">
              <div class="pse-section-title">
                ...
                <span style="font-size:12px; color:var(--pse-accent-rose); font-weight:normal; display:block; margin-top:4px;">⚠️ 다른 임베딩 모델이 생성하는 벡터는 완전히 호환되지 않습니다. 모델을 변경할 경우 반드시 아래 캐시 데이터를 먼저 지우세요.</span>
              </div>
              <label class="pse-label">제공자</label>
              ...
              <label class="pse-label" style="margin-top:10px;">요청 모드 (Concurrency)</label>
              <select id="embedding_concurrency" class="pse-input">
                <option value="1" ${Number(configCache.embedding_concurrency) === 1 ? "selected" : ""}>병렬 (복수 요청 동시 발송 허용)</option>
                <option value="0" ${Number(configCache.embedding_concurrency) === 0 ? "selected" : ""}>순차 (단일 스레드 요청만 가능)</option>
              </select>
            </div>
          </div>
          ...
          <div class="pse-page" data-page="2">
            <div class="pse-section">
              <div class="pse-section-title">로어북 쓰기 호출 설정</div>
              <div style="margin-bottom:6px;padding:7px 10px;border-radius:6px;background:rgba(180,140,0,0.1);border:1px solid rgba(180,140,0,0.25);font-size:var(--pse-font-size-small);color:var(--pse-muted);">
                ⚠️ Agent 작동 중에는 현재 대화의 로컬 로어북을 수동으로 편집하지 마세요. 시스템에 의해 자동으로 덮어쓰일 수 있습니다.
              </div>
              <div id="model_call_list" class="pse-entry-list"></div>
              <button id="add_model_call" class="pse-add-entry" type="button">호출 작업 추가</button>
            </div>
          </div>

          <div class="pse-page" data-page="3">
            <div class="pse-section">
              <label class="pse-label">시스템 앵커 프롬프트 (System Prompt)</label>
              <div class="pse-textarea-wrap">
                ...
                <button id="advanced_model_anchor_prompt_expand" class="pse-expand-btn" type="button" aria-label="확대 편집">
              ...
              <label class="pse-label">어시스턴트 프리필 (Assistant Prefill)</label>
              <div class="pse-textarea-wrap">
                <textarea id="advanced_prefill_prompt" class="pse-textarea">${escapeHtml(configCache.advanced_prefill_prompt || "")}</textarea>
                <button id="advanced_prefill_prompt_expand" class="pse-expand-btn" type="button" aria-label="확대 편집">
                  <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 4H5a1 1 0 0 0-1 1v3"/><path d="M16 4h3a1 1 0 0 1 1 1v3"/><path d="M20 16v3a1 1 0 0 1-1 1h-3"/><path d="M4 16v3a1 1 0 0 0 1 1h3"/></svg>
                </button>
              </div>
              <label class="pse-label">사전 응답 프롬프트 (Assistant Pre-Reply)</label>
              <div class="pse-textarea-wrap">
                <textarea id="advanced_prereply_prompt" class="pse-textarea">${escapeHtml(configCache.advanced_prereply_prompt || "")}</textarea>
                <button id="advanced_prereply_prompt_expand" class="pse-expand-btn" type="button" aria-label="확대 편집">
                  <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 4H5a1 1 0 0 0-1 1v3"/><path d="M16 4h3a1 1 0 0 1 1 1v3"/><path d="M20 16v3a1 1 0 0 1-1 1h-3"/><path d="M4 16v3a1 1 0 0 0 1 1h3"/></svg>
                </button>
              </div>
            </div>
          ...
          <div class="pse-page" data-page="5">
            <div class="pse-section" style="border-left:4px solid var(--pse-accent-indigo); background:linear-gradient(180deg, rgba(79, 70, 229, 0.08) 0%, var(--pse-input-bg) 100%);">
              <div class="pse-section-title">벡터 검색 (Vector Search)</div>
              <select id="vector_search_enabled" class="pse-input">
                <option value="1" ${Number(configCache.vector_search_enabled) === 1 ? "selected" : ""}>활성화 (의미 관련성 기반 로어북 항목 선택)</option>
                <option value="0" ${Number(configCache.vector_search_enabled) === 1 ? "" : "selected"}>비활성화 (전통적인 키워드 매칭 사용)</option>
              </select>
              <label class="pse-label">최근 몇 라운드 대화를 검색 기준으로 사용</label>
              ...
              <label class="pse-label">Top K (반환 항목 수)</label>
              ...
              <label class="pse-label">최소 유사도 점수 임계값 (0~1)</label>
            ...
            <div class="pse-section" style="border-left:4px solid var(--pse-accent-cyan); background:linear-gradient(180deg, rgba(8, 145, 178, 0.08) 0%, var(--pse-input-bg) 100%);">
              <div class="pse-section-title">분류 모델 설정</div>
              <label class="pse-label">사용 모델</label>
              <select id="init_bootstrap_target_model" class="pse-input">
                <option value="A" ${safeTrim(configCache.init_bootstrap_target_model) === "B" ? "" : "selected"}>주요 모델 (Main)</option>
                <option value="B" ${safeTrim(configCache.init_bootstrap_target_model) === "B" ? "selected" : ""}>보조 모델 (Aux)</option>
              </select>
              <label class="pse-label">분류 앵커 프롬프트</label>
              <div class="pse-textarea-wrap">
                ...
                <button id="init_bootstrap_model_anchor_prompt_expand" class="pse-expand-btn" type="button" aria-label="확대 편집">
                  <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 4H5a1 1 0 0 0-1 1v3"/><path d="M16 4h3a1 1 0 0 1 1 1v3"/><path d="M20 16v3a1 1 0 0 1-1 1h-3"/><path d="M4 16v3a1 1 0 0 0 1 1h3"/></svg>
                </button>
              </div>
            </div>
          ...
          <div class="pse-page" data-page="6">
            <div class="pse-section">
              <div class="pse-section-title">캐시 관리 센터 (Cache Hub)</div>
              <div class="pse-row" style="margin-bottom:8px;">
                <button id="pse-embed-cache-refresh" class="pse-btn cache" type="button" style="flex:0 0 auto;">캐시 목록 새로고침</button>
                <button id="pse-embed-cache-clear-all" class="pse-btn" type="button" style="flex:0 0 auto;background:#b44;">전체 캐시 삭제</button>
              </div>
          ...
          <div class="pse-page" data-page="7">
            <div class="pse-section">
              <div class="pse-row" style="margin-bottom:8px;">
                <button id="pse-reset-agent-defaults" class="pse-btn cache" type="button" style="flex:0 0 auto;">기본 설정으로 초기화</button>
              </div>
              <div class="pse-assembly">
                <b>▌ 시스템 핵심: 서사 감사 및 상태 추출 엔진</b><br/>
                ...
                &bull; <b>기본값 초기화</b>: 업데이트 후 오류 발생 시 「기본값 초기화」를 클릭하여 모든 프롬프트와 설정을 복원.
              </div>
            </div>
          </div>

          <div class="pse-btn-row">
            <button id="pse-save" class="pse-btn save">설정 저장</button>
            <button id="pse-close" class="pse-btn close">닫기</button>
          </div>
        </div>
      </div>
    `;
    ...
    const renderEmbeddingCacheList = async () => {
      ...
      if (!blocks.length) {
        wrap.innerHTML = `<div class="pse-assembly">현재 벡터 캐시 데이터가 없습니다.</div>`;
        return;
      }
      wrap.innerHTML = blocks.map((b, idx) => `
        <div class="pse-entry-block" data-cache-card-key="${escapeHtml(b.cardKey)}" data-cache-parity="${idx % 2 === 0 ? "even" : "odd"}">
          <div class="pse-entry-grid" style="grid-template-columns:1fr auto;">
            <div>
              <div><b>카드</b>：${escapeHtml(b.cardName)}</div>
              <div style="margin-top:4px;"><b>항목 수</b>：${escapeHtml(String(b.entryCount))}</div>
              <div style="margin-top:4px;"><b>파일 크기</b>：${escapeHtml(formatBytes(b.sizeBytes))}</div>
            </div>
            <button class="pse-entry-remove" type="button" data-delete-cache-card="1">삭제</button>
          </div>
        </div>
      `).join("");
    };
    ...
    document.getElementById("pse-embed-cache-refresh")?.addEventListener("click", async () => {
      await renderEmbeddingCacheList();
      showStatus("벡터 캐시 목록이 업데이트되었습니다.", "info");
    });

    document.getElementById("pse-embed-cache-clear-all")?.addEventListener("click", async (e) => {
      if (!confirm("전체 벡터 캐시를 삭제하시겠습니까?\n모든 카드의 캐시 데이터가 삭제되며 다음 발송 시 재생성됩니다.")) return;
      await clearAllEmbeddingCache();
      await renderEmbeddingCacheList();
      showStatus("전체 벡터 캐시가 삭제되었습니다. 다음 발송 시 재생성됩니다.", "ok");
    });
    ...
    document.getElementById("pse-embed-cache-list")?.addEventListener("click", async (e) => {
      ...
      if (store.cards && store.cards[cardKey]) {
        ...
        embeddingCacheStore = null;
        // 수정된 store를 명시적으로 전달, saveEmbeddingCacheStore가 메모리 캐시를 다시 읽어 데이터를 덮어쓰는 것을 방지
        try { await Risuai.pluginStorage.removeItem(VCACHE_CARD_PREFIX + cardKey); } catch { }
        await saveEmbeddingCacheStore(store);
        // step0 상태 초기화, 다음 발송 시 강제 재빌드
        try { await Risuai.pluginStorage.removeItem("static_knowledge_chunks"); } catch { }
        try { await Risuai.pluginStorage.removeItem("static_data_hash"); } catch { }
        try { await Risuai.pluginStorage.removeItem("step0_complete"); } catch { }
        embeddingVectorCache.clear();
      ...
      await renderEmbeddingCacheList();
      showStatus("해당 카드의 벡터 캐시가 삭제되었습니다.", "ok");
    });
      ...
      overlay.innerHTML = `
        <div class="pse-editor-modal">
          <div class="pse-editor-head">
            ...
            <button id="pse-editor-close" class="pse-editor-close" type="button" aria-label="닫기">✕</button>
          </div>
          <textarea id="pse-editor-textarea" class="pse-editor-textarea"></textarea>
          <div class="pse-editor-actions">
            <button id="pse-editor-cancel" class="pse-btn close" type="button" style="flex:0 0 auto">취소</button>
            <button id="pse-editor-apply" class="pse-btn save" type="button" style="flex:0 0 auto">적용</button>
          </div>
        </div>
      `;
    ...
    bindPromptExpander("advanced_model_anchor_prompt_expand", "advanced_model_anchor_prompt", "시스템 앵커 프롬프트 - 확대 편집");
    bindPromptExpander("advanced_prefill_prompt_expand", "advanced_prefill_prompt", "어시스턴트 프리필 - 확대 편집");
    bindPromptExpander("advanced_prereply_prompt_expand", "advanced_prereply_prompt", "사전 응답 프롬프트 - 확대 편집");
    bindPromptExpander("init_bootstrap_model_anchor_prompt_expand", "init_bootstrap_model_anchor_prompt", "분류 앵커 프롬프트 - 확대 편집");
    ...
    document.querySelector(".pse-card").addEventListener("click", (e) => {
      ...
      if (textarea) {
        let title = "프롬프트 내용 편집";
        if (btn.classList.contains("pse-entry-format-expand")) title = "출력 형식 (JSON Schema) - 확대 편집";
        ...
        return `
          <div class="pse-entry-block" data-entry-index="${entryIndex}">
            <div class="pse-entry-grid">
              <div class="pse-entry-col"><label class="pse-label">로어북 항목</label><input class="pse-input pse-entry-lore" value="${escapeHtml(e.lorebook_name)}" /></div>
              <div class="pse-entry-col"><label class="pse-label">쓰기 모드 (덮어쓰기/추가)</label><select class="pse-input pse-entry-mode">${renderSelectOptions(LORE_WRITE_MODE_OPTIONS, e.write_mode)}</select></div>
              <div class="pse-entry-col"><label class="pse-label">항상 활성화</label><select class="pse-input pse-entry-always-active"><option value="1" ${e.always_active ? "selected" : ""}>예</option><option value="0" ${!e.always_active ? "selected" : ""}>아니오</option></select></div>
            ...
            <div class="pse-entry-grid-row2">
              <div class="pse-entry-col">
                <label class="pse-label">출력 형식 (JSON Schema)</label>
                <div class="pse-textarea-wrap">
                  <textarea class="pse-entry-format-input pse-entry-format pse-textarea">${escapeHtml(String(e.output_format || ""))}</textarea>
                  <button class="pse-expand-btn pse-entry-format-expand" type="button" aria-label="확대 편집"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 4H5a1 1 0 0 0-1 1v3"/><path d="M16 4h3a1 1 0 0 1 1 1v3"/><path d="M20 16v3a1 1 0 0 1-1 1h-3"/><path d="M4 16v3a1 1 0 0 0 1 1h3"/></svg></button>
                </div>
              </div>
            </div>
            <div class="pse-entry-retention-row" style="display:${e.write_mode === "append" ? "flex" : "none"};align-items:center;gap:8px;flex-wrap:wrap;margin-top:4px;">
              <input class="pse-entry-retention-enabled" type="checkbox" id="ret_${callIndex}_${entryIndex}" ${e.retention_enabled ? "checked" : ""}
                title="활성화 후 지정 턴 수에 도달하면 이 항목의 오래된 데이터를 자동으로 정리하고 최신 몇 턴 내용만 유지합니다" />
              <label for="ret_${callIndex}_${entryIndex}" class="pse-label" style="cursor:pointer;user-select:none;">경과 후</label>
              <input class="pse-input pse-entry-retention-after" type="number" min="0" value="${e.retention_after ?? 0}"
                style="width:60px;" title="이 턴 수를 초과하면 정리 시작 (0 = 즉시)" ${e.retention_enabled ? "" : "disabled"} />
              <label class="pse-label">턴 후 자동 정리 실행, 최신 데이터만 유지</label>
              <input class="pse-input pse-entry-retention-keep" type="number" min="0" value="${e.retention_keep ?? 5}"
                style="width:60px;" title="정리 후 유지할 최신 턴 블록 수 (0 = 전체 삭제)" ${e.retention_enabled ? "" : "disabled"} />
              <label class="pse-label">턴 데이터</label>
            </div>
          </div>
        `;
      }).join("");
          ...
          <div class="pse-call-card" data-call-index="${callIndex}" data-call-target="${escapeHtml(call.target_model)}" data-call-parity="${callIndex % 2 === 0 ? "even" : "odd"}">
            <div class="pse-call-head">
              <div class="pse-entry-col" style="display:flex;align-items:center;gap:6px;">
                <input class="pse-call-allow-parallel" type="checkbox" ${call.allow_parallel ? "checked" : ""} title="활성화 후 이 작업이 다른 병렬 작업과 동시에 실행되어 대기 시간을 단축합니다." style="margin:0;flex-shrink:0;" />
                <label class="pse-label" style="margin:0;cursor:pointer;white-space:nowrap;">병렬 실행</label>
              </div>
              <div class="pse-entry-col"><label class="pse-label">호출 메모</label><input class="pse-input pse-call-name" value="${escapeHtml(call.name)}" /></div>
              <div class="pse-entry-col">
                <label class="pse-label">모델</label>
                <select class="pse-input pse-call-target"><option value="A" ${call.target_model === "A" ? "selected" : ""}>주요 모델</option><option value="B" ${call.target_model === "B" ? "selected" : ""}>보조 모델</option></select>
              </div>
              <div class="pse-entry-col"><label class="pse-label">트리거 빈도 (턴)</label><input class="pse-input pse-call-frequency" type="number" min="1" value="${String(call.every_n_turns || 1)}" /></div>
            ...
            <div class="pse-call-row2">
              <div class="pse-entry-col"><label class="pse-label">대화 턴 수 읽기 (0=전부)</label><input class="pse-input pse-call-read-rounds" type="number" min="0" value="${String(Math.max(0, toInt(call.read_dialogue_rounds, 4)))}" /></div>
              <div class="pse-entry-col"><label class="pse-label">연관 로어북 항목 (쉼표로 구분)</label><input class="pse-input pse-call-read-lorebook-names" value="${escapeHtml(String(call.read_lorebook_names || ""))}" /></div>
            </div>
            <div class="pse-entry-list">${renderCallEntries(call, callIndex)}</div>
            <button class="pse-add-entry" type="button" data-add-entry="1" data-call-index="${callIndex}">쓰기 항목 추가</button>
          </div>
        `;
      }).join("");
    };
    ...
    const readModelCallsFromUI = () => {
      ...
      return calls.length ? calls : [normalizeModelCall({ name: "호출 메모A", target_model: "A", every_n_turns: 1, read_dialogue_rounds: 4, read_lorebook_names: "", entries: [defaultOutputEntry("A")] }, 0)];
    };
    ...
    document.getElementById("add_model_call")?.addEventListener("click", () => {
      ...
      uiModelCalls.push(normalizeModelCall({ id: `call_${Date.now()}_${nextIndex}`, name: `호출 메모${nextIndex + 1}`, target_model: nextIndex % 2 === 0 ? "A" : "B", every_n_turns: 1, read_dialogue_rounds: 4, read_lorebook_names: "", entries: [defaultOutputEntry(nextIndex % 2 === 0 ? "A" : "B")] }, nextIndex));
      renderModelCalls();
    });
    ...
    modelCallListEl?.addEventListener("click", (e) => {
      ...
      if (removeCallBtn) {
        ...
        if (Number.isFinite(idx)) {
          ...
          if (uiModelCalls.length === 0) uiModelCalls = [normalizeModelCall({ name: "호출 메모A", target_model: "A", every_n_turns: 1, read_dialogue_rounds: 4, read_lorebook_names: "", entries: [defaultOutputEntry("A")] }, 0)];
          renderModelCalls();
        }
        return;
      }
    ...
    // write_mode 전환 시 retention 행 표시/숨김; retention 체크박스 선택 시 수치 입력 활성화/비활성화
    ...
    document.getElementById("pse-reset-agent-defaults")?.addEventListener("click", async () => {
      try {
        ...
        showStatus("Agent 기본값으로 초기화되었습니다.", "ok");
      } catch (e) { showStatus(`초기화 실패: ${e?.message || String(e)}`, "err"); }
    });
    ...
    const refreshEmbeddingPresetByProvider = async (applyPreset = true) => {
      ...
      // 전환 후 해당 공급자에서 이전에 저장된 키를 자동으로 불러옴
      if (keyEl) {
      ...
      // 공급자 전환 시 preset의 format 및 url 강제 적용
    ...
    document.getElementById("pse-save")?.addEventListener("click", async () => {
      ...
      } catch (e) { showStatus(`저장 실패: ${e?.message || String(e)}`, "err"); }
    });
  ...
  // ============================================================================
  // 9. 훅 및 Replacer 등록 (Hooks & Lifecycle)
  // ============================================================================
  ...
  async function ensureReplacerRegistered() {
    ...
    try {
      ...
      await Risuai.log(`${LOG} addRisuReplacer 실패: ${err?.message || String(err)}`);
      return false;
    }
  }
  ...
  function _replacerBody(messages, type) {
    return (async (messages, type) => {
      ...
      // type="display": 메인 모델이 응답을 완료함, retention 정리 수행 후 반환
      if (type === "display") {
      ...
      if (needsStep0 && type === "model") {
        try {
          ...
          if (isVectorEnabled) {
            await Risuai.log(`${LOG} 백그라운드 지식 베이스 초기화 시작 (Step 0)... 잠시 기다려주세요.`);
          } else {
            await Risuai.log(`${LOG} 백그라운드 지식 베이스 분류 시작 (Step 0, 키워드 모드)... 잠시 기다려주세요.`);
          }
          ...
          await Risuai.log(`${LOG} 지식 베이스 초기화 완료!`);
        } catch (step0Err) {
          const errMsg = step0Err?.message || String(step0Err);
          await Risuai.log(`${LOG} ❌ 지식 베이스 초기화 실패 중단: ${errMsg}`);
          if (isVectorEnabled) {
            throw new Error(`[RisuAI Agent] 벡터 지식 베이스 빌드 실패/타임아웃. 진행 상황이 저장되었습니다.\n오류 원인: ${errMsg}\n잠시 후 「재생성/보내기」를 다시 클릭하여 미완료 진행 상황을 계속하세요.`);
          } else {
            console.warn(`${LOG} Step0 분류 실패, fallback (information) 사용: ${errMsg}`);
            await Risuai.log(`${LOG} ⚠️ 지식 베이스 분류 실패, fallback 모드로 계속: ${errMsg}`);
          }
        }
      }
      ...
      if (isFirstMessage) {
        ...
        await Risuai.log(`${LOG} beforeRequest: 첫 메시지에서 추출 건너뜀.`);
      ...
      if (baseConversation.length === 0) {
        ...
        await Risuai.safeLocalStorage.setItem("last_lore_sync_error", "건너뜀: beforeRequest 페이로드에 사용 가능한 대화 텍스트 없음.");
        await Risuai.log(`${LOG} beforeRequest: 사용 가능한 대화 텍스트 없음.`);
        return await mergeToSystemPromptWithRewrite(messages, null, lastUserContent);
      }
      ...
      if (!usedCache) {
        ...
        if (dueCalls.length > 0) {
          await Risuai.log(`${LOG} Agent: 보조 모델을 호출하여 분석 및 데이터 추출 중...`);
          ...
          if (rejected.length > 0) {
            ...
            await abortMainModelWithAuxError(`보조 모델 실행 실패:\n${errs}`);
          }
          ...
          for (const res of results) {
            if (res.status === "fulfilled") {
              ...
              if (raw) {
                ...
                } catch (e) { await abortMainModelWithAuxError(`항목 저장 실패:\n${e.message}`); }
              }
            }
          }
      ...
      if (!extractedData && !usedCache) {
        ...
        if (due.length > 0) await Risuai.log(`${LOG} 경고: 보조 모델이 유효한 내용을 반환하지 않았거나 형식 파싱에 실패했습니다.`);
      }
  ...
  async function getConversationFromCurrentChat(limit) {
    ...
    for (const m of src) {
      ...
      if (!content) continue;
      // 첫 메시지 건너뜀: 첫 사용자 턴 이전의 모든 어시스턴트 턴
  ...
  try {
    ...
    await Risuai.log(`${LOG} 로드됨 (${PLUGIN_NAME} v${PLUGIN_VER}).`);
    console.log(`${LOG} 로드 완료`);
  } catch (err) {
    console.error(`${LOG} 초기화 실패:`, err);
    try { await Risuai.log(`${LOG} 실패: ${err?.message || String(err)}`); } catch { }
  }
