...
(async () => {
 ...
 err_persona_pair_missing: (missing) => `Persona core pair missing: ${missing}. Require both rp_persona_inventory and rp_character_core.`,
...
- Cast Check: Use ra_cast_state or ra_scene_state.characters. Do not include absent characters.
...
7. Write output. Express only one dominant facet per character.</textarea> <button class="pse-btn pse-copy-sql-btn" type="button" style="width:100%;padding:6px;font-size:12px;background:var(--pse-accent-greyblue);"> Copy System Prompt</button> </div> </div>`,
 ...
 err_persona_pair_missing: (missing) => `페르소나 핵심 쌍 누락: ${missing}. rp_persona_inventory와 rp_character_core 둘 다 필요합니다.`,
...
7. Write output. Express only one dominant facet per character.</textarea> <button class="pse-btn pse-copy-sql-btn" type="button" style="width:100%;padding:6px;font-size:12px;background:var(--pse-accent-greyblue);"> 시스템 프롬프트 복사</button> </div> </div>`,
 ...
 err_persona_pair_missing: (missing) => `人格核心配對缺失：${missing}。必須同時有 rp_persona_inventory 與 rp_character_core。`,
...
7. Write output. Express only one dominant facet per character.`;
 ...
 "read_lorebook_names": "ra_turn_trace, ra_scene_state, ra_director, ra_inventory",
 "read_persona_names": "",
 "entries": [
 {
 ...
 "write_mode": "overwrite",
 "always_active": true,
 "output_format": "ENTRY: ra_director\nSCHEMA — ra_director:\n{\n  \"ra_director\": {\n    \"staleness_level\": 0,\n    \"environment_intervention\": null\n  }\n}\nFIELD RULES:\n- Compare this turn's ra_turn_trace with the previous 2 turns.\n- staleness_level: 0 (completely different scene/action) to 10 (nearly identical).\n- environment_intervention: if staleness_level >= 8, write one brief unexpected environmental event that could break the pattern (arrival, noise, discovery, weather). null otherwise.\nOUTPUT: Combine all schemas into one JSON object: {\"ra_turn_trace\": {...}, \"ra_scene_state\": {...}, \"ra_inventory\": {...}, \"ra_director\": {...}}. Single line, no markdown fences, no explanation.",
 "retention_enabled": false,
 "retention_after": 0,
 "retention_keep": 0
 }
 ...
 "read_lorebook_names": "ra_logic_state, ra_quest_log, ra_scene_state, ra_turn_trace, ra_inventory",
 "read_persona_names": "",
 "entries": [
 {
 ...
 "every_n_turns": 3,
 "read_dialogue_rounds": 3,
 "read_lorebook_names": "ra_world_log, ra_world_encyclopedia, ra_turn_trace, ra_scene_state",
 "read_persona_names": "",
 "entries": [
 {
 ...
 "write_mode": "append",
 "always_active": false,
 "output_format": "CALL ROLE: Preserve new facts and durable continuity that should survive beyond the actor's visible 5 rounds.\nENTRY: ra_world_log\nTEMPORAL NOTE: You are writing entries for THIS batch of turns. Cover all turns since this call last ran.\nSCHEMA — ra_world_log:\n{\n  \"ra_world_log\": {\n    \"entries\": [\"<Name. Key fact. <=20 words.>\"]\n  }\n}\nFIELD RULES:\n- One string per entry: name first, then the key fact. <=20 words total per entry.\n- Only NEW facts not already in ra_world_log or ra_world_encyclopedia.\n- entries: [] if nothing new.\nAPPEND NOTE: Write only the new entries for this batch. Do not reproduce prior stored entries.",
 "retention_enabled": true,
 "retention_after": 15,
 "retention_keep": 0
 },
 ...
 "write_mode": "overwrite",
 "always_active": true,
 "output_format": "ENTRY: ra_pattern_guard\nSCHEMA — ra_pattern_guard:\n{\n  \"ra_pattern_guard\": {\n    \"staleness_level\": 0,\n    \"flagged_cliches\": [\"<overused trope or scene pattern>\"],\n    \"banned_phrases\": [\"<exact phrase appearing 2+ times>\"],\n    \"variation_hint\": \"<one short suggestion for more variety | null>\"\n  }\n}\nFIELD RULES:\n- staleness_level: 0 (fully fresh) to 10 (repetitive loop).\n- flagged_cliches: up to 3 overused tropes in recent turns. [] if none.\n- banned_phrases: exact phrases appearing 2+ times in recent assistant replies. [] if none.\n- variation_hint: null if no meaningful repetition detected.\nOUTPUT: Combine all schemas into one JSON object: {\"ra_world_log\": {...}, \"ra_turning_point_log\": {...}, \"ra_pattern_guard\": {...}}. Single line, no markdown fences, no explanation.",
 "retention_enabled": false,
 "retention_after": 0,
 "retention_keep": 0
 }
 ...
 "read_lorebook_names": "ra_arc_memory, ra_turn_trace, ra_quest_log, ra_scene_state, ra_world_log, ra_turning_point_log, ra_persistent_memory, ra_inventory",
 ...
 "every_n_turns": 15,
 "read_dialogue_rounds": 1,
 "read_lorebook_names": "ra_world_log, ra_world_encyclopedia, ra_arc_memory",
 "read_persona_names": "",
 "entries": [
 {
 ...
 "every_n_turns": 1,
 "read_dialogue_rounds": 2,
 "read_lorebook_names": "ra_turn_trace, ra_scene_state, ra_director, ra_quest_log, ra_inventory",
 "read_persona_names": "",
 "entries": [
 {
 "lorebook_name": "ra_turn_trace",
 ...
 "write_mode": "overwrite",
 "always_active": true,
 "output_format": "ENTRY: ra_director\nSCHEMA — ra_director:\n{\n  \"ra_director\": {\n    \"staleness_level\": 0,\n    \"strategic_stagnation\": false,\n    \"global_stagnation\": false,\n    \"environment_intervention\": null\n  }\n}\nFIELD RULES:\n- Compare this turn's ra_turn_trace with the previous 2 turns.\n- staleness_level: 0 (completely different) to 10 (nearly identical location, action, stakes).\n- strategic_stagnation: true when ra_quest_log active_threads have not progressed for 3+ consecutive turns.\n- global_stagnation: true when BOTH staleness_level >= 7 AND strategic_stagnation is true.\n- environment_intervention: if staleness_level >= 8, write a brief unexpected event to break the pattern. null otherwise.\nOUTPUT: Combine all schemas into one JSON object: {\"ra_turn_trace\": {...}, \"ra_scene_state\": {...}, \"ra_inventory\": {...}, \"ra_director\": {...}}. Single line, no markdown fences, no explanation.",
 "retention_enabled": false,
 "retention_after": 0,
 "retention_keep": 0
 }
 ...
 "every_n_turns": 1,
 "read_dialogue_rounds": 3,
 "read_lorebook_names": "ra_knowledge_matrix, ra_logic_state, ra_quest_log, ra_scene_state, ra_turn_trace, ra_inventory",
 "read_persona_names": "",
 "entries": [
 {
 ...
 "write_mode": "overwrite",
 "always_active": true,
 "output_format": "ENTRY: ra_logic_state\nSCHEMA — ra_logic_state:\n{\n  \"ra_logic_state\": {\n    \"logic_violation\": \"<describe contradiction | null>\",\n    \"strict_directive\": \"<Proceed normally | Reject the user action and narrate failure | Show cognitive friction: character hesitates or struggles>\"\n  }\n}\nFIELD RULES:\n- Check user_move against player_state, location, and known facts from ra_scene_state.\n- Violation found: describe it concisely; set strict_directive to the appropriate enforcement value.\n- No violation: logic_violation: null; strict_directive: \"Proceed normally\".",
 "retention_enabled": false,
 "retention_after": 0,
 "retention_keep": 0
 },
 ...
 "every_n_turns": 2,
 "read_dialogue_rounds": 2,
 "read_lorebook_names": "ra_strategy_layer, ra_knowledge_matrix, ra_strategy_analysis, ra_scene_state, ra_quest_log, ra_inventory",
 "read_persona_names": "",
 "entries": [
 {
 ...
 "read_lorebook_names": "ra_knowledge_matrix, ra_knowledge_annotations, ra_strategy_layer, ra_strategy_analysis, ra_quest_log, ra_turn_trace, ra_scene_state, ra_inventory",
 "read_persona_names": "",
 "entries": [
 {
 ...
 "write_mode": "overwrite",
 "always_active": true,
 "output_format": "ENTRY: ra_pattern_guard\nSCHEMA — ra_pattern_guard:\n{\n  \"ra_pattern_guard\": {\n    \"staleness_level\": 0,\n    \"flagged_cliches\": [\"<overused trope>\"],\n    \"banned_phrases\": [\"<exact phrase 2+ times>\"],\n    \"variation_hint\": \"<one short suggestion for more variety | null>\",\n    \"variation_direction\": \"<none|more_dialogue|more_action|shorter|longer|more_subtext>\"\n  }\n}\nFIELD RULES:\n- staleness_level: narrative freshness 0 (totally fresh) to 10 (repetitive loop).\n- flagged_cliches: up to 3. [] if none. banned_phrases: [] if none.\n- variation_hint: null if no meaningful repetition detected.\n- variation_direction: \"none\" if no change needed.\nOUTPUT: Combine all schemas into one JSON object: {\"ra_knowledge_annotations\": {...}, \"ra_strategy_analysis\": {...}, \"ra_pattern_guard\": {...}}. Single line, no markdown fences, no explanation.",
 "retention_enabled": false,
 "retention_after": 0,
 "retention_keep": 0
 }
 ...
 "write_mode": "append",
 "always_active": false,
 "output_format": "ENTRY: ra_turning_point_log\nTEMPORAL NOTE: You are writing entries for THIS batch of turns. Cover all turns since this call last ran.\nSCHEMA — ra_turning_point_log:\n{\n  \"ra_turning_point_log\": {\n    \"entries\": [\n      {\n        \"turning_point_detected\": \"<true|false>\",\n        \"event\": \"<concise event or line | null>\",\n        \"affected_characters\": [\"<CharName or role>\"],\n        \"what_must_still_remain_true\": [\"<canon fact that must not be denied later>\"],\n        \"carry_forward_details\": [\"<concrete reminder for future scenes>\"],\n        \"inventory_updates\": [\"<description of clothing changes or important obtained items>\"],\n        \"reentry_hook\": \"<how this should reactivate on return | null>\"\n      }\n    ]\n  }\n}\nFIELD RULES:\n- entries: [] if nothing beyond the latest 5 rounds needs preserving.\n- Record only changes future scenes must restore: leverage swings, exposed knowledge, obligations, reveals, losses, or other canon-locking details.\n- Keep carry_forward_details to 2-4 short items per entry.\n- inventory_updates: Only log permanent or significant item acquisitions and explicit clothing changes that future scenes must remember.\n- Do not log abstract themes or generic emotions.\nAPPEND NOTE: Write only the new turning-point entries for this batch. Do not reproduce prior entries.",
 "retention_enabled": true,
 "retention_after": 40,
 "retention_keep": 6
 }
 ...
 "every_n_turns": 10,
 "read_dialogue_rounds": 1,
 "read_lorebook_names": "ra_arc_memory, ra_turn_trace, ra_quest_log, ra_strategy_layer, ra_strategy_analysis, ra_knowledge_matrix, ra_world_log, ra_turning_point_log, ra_persistent_memory, ra_inventory",
 "read_persona_names": "ra_character_core",
 "entries": [
 {
 "lorebook_name": "ra_arc_memory",
 ...
 {
 "lorebook_name": "ra_knowledge_archive",
 "write_mode": "append",
 "always_active": false,
 "output_format": "ENTRY: ra_knowledge_archive\nSCHEMA — ra_knowledge_archive:\n{\n  \"ra_knowledge_archive\": {\n    \"entries\": [\n      {\n        \"id\": \"K001\",\n        \"subject\": \"<archived fact, <=15 words>\",\n        \"true_answer\": \"<truth at time of archival, <=15 words>\",\n        \"status_at_archive\": \"<locked|superseded|resolved>\",\n        \"archived_reason\": \"<why no longer active, <=12 words>\"\n      }\n    ]\n  }\n}\nFIELD RULES:\n- Move entries from ra_knowledge_matrix that are fully resolved, superseded, or no longer plot-relevant.\n- Do not archive entries still in active play.\n- entries: [] if nothing to archive.",
 "retention_enabled": true,
 "retention_after": 25,
 "retention_keep": 5
 },
 {
 "lorebook_name": "ra_strategy_archive",
 "write_mode": "append",
 "always_active": false,
 "output_format": "ENTRY: ra_strategy_archive\nSCHEMA — ra_strategy_archive:\n{\n  \"ra_strategy_archive\": {\n    \"entries\": [\n      {\n        \"goal\": \"<archived strategic goal, <=15 words>\",\n        \"outcome\": \"<how it concluded, <=12 words>\",\n        \"impact\": \"<lasting strategic consequence, <=12 words>\"\n      }\n    ]\n  }\n}\nFIELD RULES:\n- Archive completed or abandoned strategic goals from ra_strategy_layer history.\n- entries: [] if nothing resolved.\nOUTPUT: Combine all schemas into one JSON object: {\"ra_arc_memory\": {...}, \"ra_persistent_memory\": {...}, \"ra_reentry_guard\": {...}, \"ra_knowledge_archive\": {...}, \"ra_strategy_archive\": {...}}. Single line, no markdown fences, no explanation.\nAPPEND NOTE for ra_arc_memory: Write only the new arc entry for this batch. Do not reproduce prior entries.",
 "retention_enabled": true,
 "retention_after": 25,
 "retention_keep": 5
 }
 ...
 "every_n_turns": 15,
 "read_dialogue_rounds": 1,
 "read_lorebook_names": "ra_world_log, ra_world_encyclopedia, ra_arc_memory, ra_knowledge_archive",
 "read_persona_names": "",
 "entries": [
 {
 "lorebook_name": "ra_world_encyclopedia",
 ...
 "write_mode": "overwrite",
 "always_active": false,
 "output_format": "ENTRY: ra_director\nSCHEMA: ra_director:\n{\n  \"ra_director\": {\n    \"staleness_level\": 0,\n    \"loop_signal\": null\n  }\n}\nFIELD RULES:\n- Compare this turn's ra_turn_trace with the previous 2 turns.\n- staleness_level: 0 (fully different) to 10 (nearly identical scene and interaction pattern).\n- loop_signal: if staleness_level >= 8, write one sentence suggesting an element that could break the loop (interruption, revelation, entrance, shift in character approach). null otherwise. This signal is read by Persona Routing.\nOUTPUT: Combine all schemas into one JSON object: {\"ra_turn_trace\": {...}, \"ra_scene_state\": {...}, \"ra_inventory\": {...}, \"ra_director\": {...}}. Single line, no markdown fences, no explanation.",
 "retention_enabled": false,
 "retention_after": 0,
 "retention_keep": 0
 }
 ...
 "write_mode": "overwrite",
 "always_active": true,
 "output_format": "CALL ROLE: Enforce continuity and keep active story threads usable after older chat scrolls away.\nENTRY: ra_logic_state\nSCHEMA: ra_logic_state:\n{\n  \"ra_logic_state\": {\n    \"logic_violation\": \"<describe contradiction | null>\",\n    \"strict_directive\": \"<Proceed normally | Reject the user action and narrate failure | Show cognitive friction: character hesitates or struggles>\",\n    \"entrance_signal\": {\n      \"character\": \"<CharName | null>\",\n      \"source_thread\": \"<which ra_quest_log thread demands this | null>\",\n      \"mode\": \"<incidental|urgent|background>\",\n      \"binding\": \"<suggested|required>\",\n      \"blocked\": false\n    }\n  }\n}\nFIELD RULES for entrance_signal:\n- Scan ra_quest_log.active_threads. If any active (not stalled) thread has requires_absent non-null, populate entrance_signal.\n- If multiple qualify, pick highest narrative urgency.\n- blocked: true when ra_scene_state.scene_type is combat, ceremony, or pursuit AND stakes_level >= 6. When blocked, character MUST be null.\n- binding: \"required\" only when the thread cannot proceed without this character. Otherwise \"suggested\".\n- All fields null when no absent character is needed or when blocked is true.",
 "retention_enabled": false,
 "retention_after": 0,
 "retention_keep": 0
 },
 ...
 "every_n_turns": 1,
 "read_dialogue_rounds": 2,
 "read_lorebook_names": "ra_scene_state, ra_logic_state, ra_persistent_memory, ra_reentry_guard, ra_facet_audit, ra_persona_evolution, ra_turning_point_log, ra_director, ra_inventory, ra_quest_log",
 ...
 "every_n_turns": 1,
 "read_dialogue_rounds": 2,
 "read_lorebook_names": "ra_turn_advice, ra_facet_audit",
 "read_persona_names": "ra_persona_inventory",
 "entries": [
 {
 ...
 "every_n_turns": 10,
 "read_dialogue_rounds": 1,
 "read_lorebook_names": "ra_arc_memory, ra_turn_trace, ra_quest_log, ra_scene_state, ra_world_log, ra_turning_point_log, ra_persistent_memory, ra_persona_evolution, ra_facet_audit, ra_inventory",
 "read_persona_names": "ra_character_core",
 "entries": [
 {
 "lorebook_name": "ra_arc_memory",
 ...
 "every_n_turns": 1,
 "read_dialogue_rounds": 2,
 "read_lorebook_names": "ra_turn_trace, ra_scene_state, ra_cast_state, ra_director, ra_inventory",
 "read_persona_names": "ra_character_core",
 "entries": [
 {
 "lorebook_name": "ra_turn_trace",
 ...
 {
 "lorebook_name": "ra_cast_state",
 "write_mode": "overwrite",
 "always_active": true,
 "output_format": "ENTRY: ra_cast_state\nTEMPORAL NOTE: ra_cast_state in the lorebook is from T-1. Overwrite with CURRENT scene's roster.\nSCHEMA: ra_cast_state:\n{\n  \"ra_cast_state\": {\n    \"present_characters\": [\n      {\n        \"name\": \"<CharName>\",\n        \"status\": \"<active|observing|entering|departing>\",\n        \"brief_stance\": \"<current posture toward the scene, <=10 words>\"\n      }\n    ]\n  }\n}\nFIELD RULES:\n- List every named character currently in the scene. Exclude player character.\n- entering: arriving this turn; departing: leaving this turn.\n- brief_stance: what role or position this character holds right now in the scene.",
 "retention_enabled": false,
 "retention_after": 0,
 "retention_keep": 0
 },
 ...
 "write_mode": "overwrite",
 "always_active": false,
 "output_format": "ENTRY: ra_director\nSCHEMA: ra_director:\n{\n  \"ra_director\": {\n    \"staleness_level\": 0,\n    \"loop_signal\": null\n  }\n}\nFIELD RULES:\n- Compare this turn's ra_turn_trace with the previous 2 turns.\n- staleness_level: 0 (fully different) to 10 (nearly identical scene and interaction pattern).\n- loop_signal: if staleness_level >= 8, write one sentence suggesting an element that could break the loop. null otherwise.\nOUTPUT: Combine all schemas into one JSON object: {\"ra_turn_trace\": {...}, \"ra_scene_state\": {...}, \"ra_cast_state\": {...}, \"ra_inventory\": {...}, \"ra_director\": {...}}. Single line, no markdown fences, no explanation.",
 "retention_enabled": false,
 "retention_after": 0,
 "retention_keep": 0
 }
 ...
 "every_n_turns": 1,
 "read_dialogue_rounds": 3,
 "read_lorebook_names": "ra_knowledge_matrix, ra_logic_state, ra_quest_log, ra_scene_state, ra_cast_state, ra_turn_trace, ra_inventory",
 "read_persona_names": "",
 "entries": [
 {
 "lorebook_name": "ra_knowledge_matrix",
 ...
 "every_n_turns": 1,
 "read_dialogue_rounds": 2,
 "read_lorebook_names": "ra_scene_state, ra_logic_state, ra_knowledge_matrix, ra_mask_state, ra_persistent_memory, ra_reentry_guard, ra_facet_audit, ra_persona_evolution, ra_turning_point_log, ra_director, ra_inventory, ra_quest_log, ra_cast_state",
 "read_persona_names": "ra_persona_inventory",
 "entries": [
 {
 "lorebook_name": "ra_persona_importance",
 ...
 {
 "id": "call_strategy_guard",
 "name": "Strategy & Guard",
 "target_model": "B",
 "every_n_turns": 2,
 "read_dialogue_rounds": 3,
 "read_lorebook_names": "ra_scene_state, ra_cast_state, ra_knowledge_matrix, ra_strategy_layer, ra_relation_web, ra_turn_advice, ra_response_guard, ra_turn_trace, ra_quest_log, ra_inventory",
 "read_persona_names": "",
 "entries": [
 {
 "lorebook_name": "ra_strategy_layer",
 "write_mode": "overwrite",
 "always_active": true,
 "output_format": "CALL ROLE: Summarize strategy, concealment, final reply risks, and repetition pressure for this phase.\nENTRY: ra_strategy_layer\nSCHEMA: ra_strategy_layer:\n{\n  \"ra_strategy_layer\": {\n    \"player_side_goal\": \"<current strategic goal, <=15 words>\",\n    \"player_side_operations\": [\"<active operation or line of action>\"],\n    \"rival_operations\": [\"<opposing move or pressure>\"],\n    \"immediate_leverage\": [\"<asset|weakness|debt|secret|positional advantage>\"],\n    \"strategic_risk\": \"<main current risk, <=18 words | null>\"\n  }\n}\nFIELD RULES:\n- Focus on active, decision-relevant strategic moves only.\n- TEMPORAL NOTE: Update based on this turn's latest dialogue developments.\n- [] for empty lists. null for absent fields.",
 "retention_enabled": false,
 "retention_after": 0,
 "retention_keep": 0
 },
 ...
 "write_mode": "overwrite",
 "always_active": true,
 "output_format": "ENTRY: ra_pattern_guard\nSCHEMA: ra_pattern_guard:\n{\n  \"ra_pattern_guard\": {\n    \"staleness_level\": 0,\n    \"dominant_recent_pattern\": \"<recurring scene structure or style, <=18 words | null>\",\n    \"dominant_persona_loop\": \"<repeating character mask or reaction, <=18 words | null>\",\n    \"flagged_cliches\": [\"<overused trope>\"],\n    \"banned_phrases\": [\"<exact phrase 2+ times>\"],\n    \"variation_hint\": \"<one short suggestion for more variety | null>\",\n    \"variation_direction\": \"<none|more_dialogue|more_subtext|less_exposition|less_inner_monologue|more_action|more_formality|more_indirectness|shorter|longer>\",\n    \"persona_variation_hint\": \"<alternative valid expression of current persona | null>\"\n  }\n}\nFIELD RULES:\n- staleness_level: 0 (fully fresh) to 10 (repetitive loop).\n- dominant_persona_loop: flag when the same mask or political reaction pattern dominates multiple recent turns.\n- flagged_cliches: [] if none. banned_phrases: [] if none.\n- variation_hint: null if no meaningful repetition detected.\n- variation_direction: \"none\" if no change needed.\nOUTPUT: Combine all schemas into one JSON object: {\"ra_strategy_layer\": {...}, \"ra_mask_state\": {...}, \"ra_response_guard\": {...}, \"ra_pattern_guard\": {...}}. Single line, no markdown fences, no explanation.",
 "retention_enabled": false,
 "retention_after": 0,
 "retention_keep": 0
 }
 ...
 "write_mode": "append",
 "always_active": false,
 "output_format": "ENTRY: ra_turning_point_log\nTEMPORAL NOTE: You are writing entries for THIS batch of turns. Cover all turns since this call last ran.\nSCHEMA: ra_turning_point_log:\n{\n  \"ra_turning_point_log\": {\n    \"entries\": [\n      {\n        \"turning_point_detected\": \"<true|false>\",\n        \"event\": \"<concise event or line | null>\",\n        \"affected_characters\": [\"<CharName or role>\"],\n        \"what_must_still_remain_true\": [\"<canon fact that must not be denied later>\"],\n        \"carry_forward_details\": [\"<concrete reminder for future scenes>\"],\n        \"inventory_updates\": [\"<description of clothing changes or important obtained items>\"],\n        \"reentry_hook\": \"<how this should reactivate on return | null>\"\n      }\n    ]\n  }\n}\nFIELD RULES:\n- entries: [] if nothing beyond the latest 5 rounds needs preserving.\n- Record only changes future scenes must restore: leverage swings, exposed alliances, mask damage, obligations, reveals, losses, or other canon-locking details.\n- Keep carry_forward_details to 2-4 short items per entry.\n- inventory_updates: Only log permanent or significant item acquisitions and explicit clothing changes that future scenes must remember.\n- Do not log abstract themes or generic emotions.\nAPPEND NOTE: Write only the new turning-point entries for this batch. Do not reproduce prior entries.",
 "retention_enabled": true,
 "retention_after": 40,
 "retention_keep": 6
 }
 ...
 "every_n_turns": 4,
 "read_dialogue_rounds": 5,
 "read_lorebook_names": "ra_turn_trace, ra_scene_state, ra_cast_state, ra_persona_importance, ra_knowledge_matrix, ra_strategy_layer, ra_mask_state, ra_arc_memory, ra_relation_web, ra_inventory",
 "read_persona_names": "",
 "entries": [
 {
 ...
 "every_n_turns": 10,
 "read_dialogue_rounds": 1,
 "read_lorebook_names": "ra_arc_memory, ra_strategy_layer, ra_relation_web, ra_knowledge_matrix, ra_persona_importance, ra_scene_principles, ra_turning_point_log, ra_persistent_memory, ra_facet_audit, ra_persona_evolution, ra_world_log, ra_inventory, ra_turn_trace, ra_quest_log",
 "read_persona_names": "ra_character_core",
 "entries": [
 {
 "lorebook_name": "ra_arc_memory",
 ...
 "every_n_turns": 15,
 "read_dialogue_rounds": 1,
 "read_lorebook_names": "ra_world_log, ra_world_encyclopedia, ra_arc_memory, ra_persistent_memory, ra_knowledge_archive",
 "read_persona_names": "",
 "entries": [
 {
 "lorebook_name": "ra_world_encyclopedia",
...
TIEBREAKERS:
- If a block contains world facts and behavior directives → "rp_instruction".
 ...
 *::-webkit-scrollbar-thumb { background:rgba(110,110,128,0.32); border-radius:99px; }
 ...
 url: endpoint.url, apiKey: endpoint.key, model: endpoint.model, format: endpoint.format, temperature: endpoint.temperature, messages: extractedMessages, timeoutMs: configCache.timeout_ms, mode: call.target_model, thinkingEnabled: endpoint.thinkingEnabled || false, thinkingLevel: endpoint.thinkingLevel || "",
 });
 ...
 if (!raw) return raw;
 if (name !== "ra_world_encyclopedia") return raw;
 let parsed;
 try {
 parsed = JSON.parse(raw);
 } catch {
 ...
 parsed.ra_world_encyclopedia &&
 typeof parsed.ra_world_encyclopedia === "object" &&
 !Array.isArray(parsed.ra_world_encyclopedia)
 ? parsed.ra_world_encyclopedia
 : parsed;
 if (!root || typeof root !== "object" || Array.isArray(root)) return raw;
 const sections = [];
 ...
 const pushItem = (heading, lines) => {
 sections.push(
 [`## ${heading}`, ...lines.filter((line) => !!safeTrim(line))].join("\n"),
 );
 };
 geography.forEach((item, idx) => {
 const nameLine = safeTrim(item?.name) || `Place ${idx + 1}`;
 pushItem(`Geography: ${nameLine}`, [
 `- name: ${nameLine}`,
 `- description: ${safeTrim(item?.description) || "(none)"}`,
 `- current_relevance: ${safeTrim(item?.current_relevance) || "(none)"}`,
 ]);
 });
 npcs.forEach((item, idx) => {
 const nameLine = safeTrim(item?.name) || `NPC ${idx + 1}`;
 pushItem(`NPC: ${nameLine}`, [
 `- name: ${nameLine}`,
 `- role: ${safeTrim(item?.role) || "(none)"}`,
 `- status: ${safeTrim(item?.status) || "(none)"}`,
 `- notes: ${safeTrim(item?.notes) || "(none)"}`,
 ]);
 });
 factions.forEach((item, idx) => {
 const nameLine = safeTrim(item?.name) || `Faction ${idx + 1}`;
 pushItem(`Faction: ${nameLine}`, [
 `- name: ${nameLine}`,
 `- description: ${safeTrim(item?.description) || "(none)"}`,
 `- relations: ${safeTrim(item?.relations) || "(none)"}`,
 ]);
 });
 lore.forEach((item, idx) => {
 const topicLine = safeTrim(item?.topic) || `Lore ${idx + 1}`;
 pushItem(`Lore: ${topicLine}`, [
 `- topic: ${topicLine}`,
 `- detail: ${safeTrim(item?.detail) || "(none)"}`,
 ]);
 });
 ...
 if (content.startsWith(KNOWLEDGE_SECTION_TAGS.rp_instruction)) return KNOWLEDGE_SECTION_TAGS.rp_instruction;
 ...
 */
 ...
 : String(toInt(configCache.active_preset, 1));
 const calls = getModelCallsByPreset(preset);
 ...
 */
 ...
 if (combined1) block1_arr.push(`${KNOWLEDGE_SECTION_TAGS.rp_instruction}\n${combined1}`);
 ...
 *::-webkit-scrollbar-thumb { background: rgba(120,120,135,0.35); border-radius: 99px; }
 ...
 .pse-entry-remove {
        ...
        transition: all 0.18s; font-size: 12px; font-weight: 600;
        padding: 0 12px;
        /* backdrop-filter removed: tearing fix inside scroll container */
        box-shadow: 0 1px 0 rgba(255,255,255,0.55) inset;
      }
 ...
 (k) => delExpectedPrefixes.some(p => k === `${p}_(${charName})` || k === `rp_persona_inventory_(${charName})` || k === `rp_character_core_(${charName})`)
 );
