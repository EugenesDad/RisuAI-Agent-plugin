...
6. Match key names and value types exactly.`,
  ...
  // ============================================================================
  // 2. 全域狀態與工具類 (Global State & Utilities)
  // ============================================================================
  const uiIds = [];
  let replacerFn = null;
  ...
  // ============================================================================
  // 3. 向量資料庫與快取 (pluginStorage Vector Cache)
  // 改用 Risuai.pluginStorage 取代 IndexedDB（iframe 沙盒內無 IDB 存取權限）
  // 採用「index + per-card」分拆策略：
  ...
  async function saveEmbeddingCacheStore(storeToSave = null) {
    ...
    try {
      ...
      try { await Risuai.log(`${LOG} 警告：向量快取存檔失敗 (${err.message})`); } catch { }
    }
  }
  ...
  async function clearAllEmbeddingCache() {
    ...
    // 2. 清除 pluginStorage 向量索引與所有 card 資料
    try {
      const idxRaw = await Risuai.pluginStorage.getItem(VCACHE_INDEX_KEY);
      if (idxRaw) {
        const idx = JSON.parse(idxRaw);
  ...
  function parsePossiblyWrappedJson(text) {
    ...
    // 2. Fallback 混合策略：
    //    先用 lastBrace（正確處理字串值中含有 } 的情況），
    //    若解析失敗則改用深度追蹤（正確處理模型輸出多個並排 JSON 物件的情況）。
  ...
  function resolveExtractorConfig() {
    ...
    // 從 provider_key_map 讀取各供應商獨立儲存的 key
  ...
  // ============================================================================
  // 5. 模型認證與外部 API 管理 (Model API & Headers)
  // ============================================================================
  ...
  // ============================================================================
  // 6. 核心文本與向量處理邏輯 (Text & Vector Algorithms)
  // ============================================================================
  ...
  async function fetchEmbeddingVectorsRemote(texts, cfg) {
    const runner = async () => {
      ...
      const allVectors = [];
      await Risuai.log(`${LOG} [Embed] 開始: provider=${cfg.provider}, model=${cfg.requestModel}, 共${input.length}筆 → ${batches.length}批次`);
      ...
      for (let bi = 0; bi < batches.length; bi++) {
        ...
        try {
          if (cfg.format === "google") {
            ...
            if (!baseUrl || !model) throw new Error("Google Embedding: URL 或 model 未設定");
            ...
            await Risuai.log(`${LOG} [Embed] Google 回應 via=${via}: status=${res.status}, ok=${res.ok}`);
            ...
            await Risuai.log(`${LOG} [Embed] Google 解析: 得到 ${batchVectors.length} 個向量`);
          } else {
            const url = normalizeEmbeddingUrl(cfg.url, cfg.format);
            const model = safeTrim(cfg.requestModel || cfg.selectedModel || "");
            if (!url || !model) throw new Error(`Embedding: URL(${url}) 或 model(${model}) 未設定`);
            ...
            await Risuai.log(`${LOG} [Embed] OpenAI-Compat 回應 via=${via}: status=${res.status}, ok=${res.ok}`);
            if (!isResponseLike(res) || !res.ok) {
              const errTxt = await readResponseErrorText(res);
            ...
            await Risuai.log(`${LOG} [Embed] OpenAI-Compat 解析: 得到 ${batchVectors.length} 個向量`);
          }
          if (batchVectors.length !== batchInput.length || batchVectors.some((v) => !Array.isArray(v) || !v.length)) {
            throw new Error(`${batchLabel} 向量數量不符 (got ${batchVectors.length}, expected ${batchInput.length})`);
          }
          allVectors.push(...batchVectors);
        } catch (e) {
          console.error(`${LOG} Embedding ${batchLabel} failed:`, e);
          await Risuai.log(`${LOG} [Embed] ❌ ${batchLabel} 失敗: ${e?.message || String(e)}`);
          for (let j = 0; j < batchInput.length; j++) allVectors.push([]);
          break;
        }
      }
      while (allVectors.length < input.length) allVectors.push([]);
      await Risuai.log(`${LOG} [Embed] 完成: 共 ${allVectors.filter(v => v && v.length).length}/${input.length} 筆成功`);
      return allVectors;
    };
  ...
  async function getEmbeddingsForTexts(texts, skipCache = false) {
    ...
    if (out.some((v) => !Array.isArray(v) || !v.length)) throw new Error("部分 embedding 向量取得失敗。");
    return out;
  }
  ...
  // ============================================================================
  // 7. 模型提取與知識注入 (Extractor & Knowledge Injection)
  // ============================================================================
  ...
  function normalizeOutputEntry(entry, target) {
    ...
    // retention_enabled：僅 write_mode=append 時有效，啟用後會自動修剪舊回合資料
    // retention_after：累積超過此回合數後才開始修剪（0 = 立即）
    // retention_keep：每次修剪後保留最新幾回合的區塊
    ...
    const retentionKeep = Math.max(0, toInt(entry?.retention_keep, 5));  // 0 = 清空全部
    return {
  ...
  function normalizeModelCall(call, index = 0) {
    ...
    const nameFallback = index === 0 ? "目的備註A" : index === 1 ? "目的備註B" : `目的備註${index + 1}`;
    const every = Math.max(1, toInt(call?.every_n_turns, 1));
    const readRounds = Math.max(0, toInt(call?.read_dialogue_rounds, 4));  // 0 = 讀取全部
    const entries = parseOutputEntries(call?.entries, target);
    // allow_parallel：允許此 call 與其他 allow_parallel call 同時發出（上限 3 個並行）
  ...
  async function writeOutputsForCall(modelCall, raw, parsed, reqHash, roundIndex = 0) {
    ...
    if (entries.length > 1 && (!parsed || typeof parsed !== "object")) {
      ...
      throw new Error(`輔助模型 (${modelCall.name}) 必須回傳 JSON 物件，但解析失敗。raw 前100字：${String(raw || "").slice(0, 100)}`);
    }
    ...
    if (pendingWrites.length === 0) {
      if (entries.length > 0) {
        await Risuai.log(`${LOG} 警告：模型回傳內容與預期的條目名稱 (${entries.map(e => e.lorebook_name).join(", ")}) 不符或格式錯誤，已略過寫入。`);
      }
      return;
    }
    ...
    if (!wrote) {
      await Risuai.log(`${LOG} 警告：將資料批次寫入至本機 Lorebook 時發生儲存異常 (call: ${modelCall.name})。`);
    }
  }
    ...
    if (configCache.vector_search_enabled === 0) {
      ...
      if (staticRaw) {
        try {
          ...
          await Risuai.log(`${LOG} ⚠️ 無法解析 static_knowledge_chunks，回退到即時抓取模式`);
        }
      } else {
      }
      ...
      if (allChunks.length === 0) {
        ...
        await Risuai.log(`${LOG} ⚠️ 無可用 chunk（static_knowledge_chunks 為空且無 dynamicChunks），知識庫注入已跳過。請確認是否已完成初始化（Step 0）。`);
      }
      ...
      try {
        ...
        if (missingTexts.length > 0) {
          ...
          for (let i = 0; i < missingTexts.length; i += batchSize) {
            ...
            await Risuai.log(`${LOG} Agent: 正在執行向量運算 (${i + 1}~${Math.min(i + batchSize, missingTexts.length)}/${missingTexts.length})...`);
  ...
  async function mergeToSystemPromptWithRewrite(messages, payload, queryText) {
    ...
    try {
      ...
      throw new Error(`知識庫注入失敗 (Knowledge Injection Failed): ${e.message}`);
    }
  ...
  async function runStep0Classification(char, resolvedGlobalNote, staticDataHash, resumeMode = false) {
    ...
    lorebook.forEach((l, idx) => {
      ...
      // RisuAI 內部用 alwaysActive，或相容 SillyTavern 的 constant 欄位表示「永遠啟用」
    ...
    if (resumeMode) {
      ...
      } catch { /* resumeMode 讀取失敗，fall through 到完整重跑 */ }
    }
    ...
    if (skipClassification || chunksToClassify.length === 0) {
      ...
      for (let i = 0; i < chunksToClassify.length; i += batchSize) {
        ...
        await Risuai.log(`${LOG} 正在進行初始資料分類 (${currentBatch}/${totalBatches})...`);
  ...
  // ============================================================================
  // 8. 介面與選項渲染 (UI & Options Rendering)
  // ============================================================================
...
.pse-call-card[data-call-parity="odd"] { border-left-color:var(--pse-accent-green); background:linear-gradient(180deg, rgba(5, 150, 105, 0.12) 0%, var(--pse-input-bg) 100%); }
  ...
  async function abortMainModelWithAuxError(message) {
    const msg = String(message || "輔助模型呼叫或處理失敗");
    await Risuai.pluginStorage.setItem("last_lore_sync_error", msg);
    await Risuai.log(`${LOG} ❌ 錯誤：${msg}。已強制中斷主模型回應！`);
    ...
    throw new Error(`[RisuAI Agent 錯誤] ${msg} \n(已為您攔截主模型請求以節省額度)`);
  }
  ...
  async function renderSettingsUI() {
        ...
        <div class="pse-card">
          ...
          <div class="pse-page active" data-page="1">
            <div class="pse-section pse-model-section-a">
              ...
              <label class="pse-label">溫度 (Temperature)</label>
              ...
              <label class="pse-label">請求模式 (Concurrency)</label>
              <select id="extractor_a_concurrency" class="pse-input">
                <option value="1" ${Number(configCache.extractor_a_concurrency) === 1 ? "selected" : ""}>併發 (允許同時發送複數請求)</option>
                <option value="0" ${Number(configCache.extractor_a_concurrency) === 0 ? "selected" : ""}>序列 (僅能單線請求)</option>
              </select>
            </div>

            <div class="pse-section pse-model-section-b">
              <div class="pse-section-title">輔助模型 (Auxiliary Model)</div>
              <label class="pse-label">提供者</label>
              ...
              <select id="extractor_b_concurrency" class="pse-input">
                <option value="1" ${Number(configCache.extractor_b_concurrency) === 1 ? "selected" : ""}>併發 (允許同時發送複數請求)</option>
                <option value="0" ${Number(configCache.extractor_b_concurrency) === 0 ? "selected" : ""}>序列 (僅能單線請求)</option>
              </select>
            </div>
            ...
            <div class="pse-section pse-model-section-embed">
              <div class="pse-section-title">
                ...
                <span style="font-size:12px; color:var(--pse-accent-rose); font-weight:normal; display:block; margin-top:4px;">⚠️ 不同嵌入模型產出的向量完全不互通。若更換模型，請務必先清除下方快取資料。</span>
              </div>
              <label class="pse-label">提供者</label>
              ...
              <label class="pse-label" style="margin-top:10px;">請求模式 (Concurrency)</label>
              <select id="embedding_concurrency" class="pse-input">
                <option value="1" ${Number(configCache.embedding_concurrency) === 1 ? "selected" : ""}>併發 (允許同時發送複數請求)</option>
                <option value="0" ${Number(configCache.embedding_concurrency) === 0 ? "selected" : ""}>序列 (僅能單線請求)</option>
              </select>
            </div>
          </div>
          ...
          <div class="pse-page" data-page="2">
            <div class="pse-section">
              <div class="pse-section-title">Lorebook 寫入呼叫設定</div>
              <div style="margin-bottom:6px;padding:7px 10px;border-radius:6px;background:rgba(180,140,0,0.1);border:1px solid rgba(180,140,0,0.25);font-size:var(--pse-font-size-small);color:var(--pse-muted);">
                ⚠️ Agent 運作期間請勿手動編輯當前對話的 Local Lorebook，以免被系統自動覆蓋。
              </div>
              <div id="model_call_list" class="pse-entry-list"></div>
              <button id="add_model_call" class="pse-add-entry" type="button">新增呼叫任務</button>
            </div>
          </div>

          <div class="pse-page" data-page="3">
            <div class="pse-section">
              <label class="pse-label">系統定位提示詞 (System Prompt)</label>
              <div class="pse-textarea-wrap">
                ...
                <button id="advanced_model_anchor_prompt_expand" class="pse-expand-btn" type="button" aria-label="放大編輯">
              ...
              <label class="pse-label">助理預填充 (Assistant Prefill)</label>
              <div class="pse-textarea-wrap">
                <textarea id="advanced_prefill_prompt" class="pse-textarea">${escapeHtml(configCache.advanced_prefill_prompt || "")}</textarea>
                <button id="advanced_prefill_prompt_expand" class="pse-expand-btn" type="button" aria-label="放大編輯">
                  <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 4H5a1 1 0 0 0-1 1v3"/><path d="M16 4h3a1 1 0 0 1 1 1v3"/><path d="M20 16v3a1 1 0 0 1-1 1h-3"/><path d="M4 16v3a1 1 0 0 0 1 1h3"/></svg>
                </button>
              </div>
              <label class="pse-label">預回覆提示詞 (Assistant Pre-Reply)</label>
              <div class="pse-textarea-wrap">
                <textarea id="advanced_prereply_prompt" class="pse-textarea">${escapeHtml(configCache.advanced_prereply_prompt || "")}</textarea>
                <button id="advanced_prereply_prompt_expand" class="pse-expand-btn" type="button" aria-label="放大編輯">
                  <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 4H5a1 1 0 0 0-1 1v3"/><path d="M16 4h3a1 1 0 0 1 1 1v3"/><path d="M20 16v3a1 1 0 0 1-1 1h-3"/><path d="M4 16v3a1 1 0 0 0 1 1h3"/></svg>
                </button>
              </div>
            </div>
          ...
          <div class="pse-page" data-page="5">
            <div class="pse-section" style="border-left:4px solid var(--pse-accent-indigo); background:linear-gradient(180deg, rgba(79, 70, 229, 0.08) 0%, var(--pse-input-bg) 100%);">
              <div class="pse-section-title">向量搜尋 (Vector Search)</div>
              <select id="vector_search_enabled" class="pse-input">
                <option value="1" ${Number(configCache.vector_search_enabled) === 1 ? "selected" : ""}>啟用 (依語意關聯度挑選 Lorebook 條目)</option>
                <option value="0" ${Number(configCache.vector_search_enabled) === 1 ? "" : "selected"}>停用 (使用傳統關鍵字匹配)</option>
              </select>
              <label class="pse-label">以最近幾輪對話作為搜尋關鍵</label>
              ...
              <label class="pse-label">Top K (回傳條目數)</label>
              ...
              <label class="pse-label">最低相似度分數門檻 (0~1)</label>
            ...
            <div class="pse-section" style="border-left:4px solid var(--pse-accent-cyan); background:linear-gradient(180deg, rgba(8, 145, 178, 0.08) 0%, var(--pse-input-bg) 100%);">
              <div class="pse-section-title">分類模型設定</div>
              <label class="pse-label">使用模型</label>
              <select id="init_bootstrap_target_model" class="pse-input">
                <option value="A" ${safeTrim(configCache.init_bootstrap_target_model) === "B" ? "" : "selected"}>主要模型 (Main)</option>
                <option value="B" ${safeTrim(configCache.init_bootstrap_target_model) === "B" ? "selected" : ""}>輔助模型 (Aux)</option>
              </select>
              <label class="pse-label">分類定位提示詞</label>
              <div class="pse-textarea-wrap">
                ...
                <button id="init_bootstrap_model_anchor_prompt_expand" class="pse-expand-btn" type="button" aria-label="放大編輯">
                  <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 4H5a1 1 0 0 0-1 1v3"/><path d="M16 4h3a1 1 0 0 1 1 1v3"/><path d="M20 16v3a1 1 0 0 1-1 1h-3"/><path d="M4 16v3a1 1 0 0 0 1 1h3"/></svg>
                </button>
              </div>
            </div>
          ...
          <div class="pse-page" data-page="6">
            <div class="pse-section">
              <div class="pse-section-title">快取管理中心 (Cache Hub)</div>
              <div class="pse-row" style="margin-bottom:8px;">
                <button id="pse-embed-cache-refresh" class="pse-btn cache" type="button" style="flex:0 0 auto;">更新快取列表</button>
                <button id="pse-embed-cache-clear-all" class="pse-btn" type="button" style="flex:0 0 auto;background:#b44;">清除全部快取</button>
              </div>
          ...
          <div class="pse-page" data-page="7">
            <div class="pse-section">
              <div class="pse-row" style="margin-bottom:8px;">
                <button id="pse-reset-agent-defaults" class="pse-btn cache" type="button" style="flex:0 0 auto;">重置為預設設定</button>
              </div>
              <div class="pse-assembly">
                <b>▌ 系統核心：敘事稽核與狀態提取引擎</b><br/>
                ...
                &bull; <b>預設重置</b>：若更新後發生錯誤，可點擊「預設重置」還原所有提示詞與配置。
              </div>
            </div>
          </div>

          <div class="pse-btn-row">
            <button id="pse-save" class="pse-btn save">儲存設定</button>
            <button id="pse-close" class="pse-btn close">關閉</button>
          </div>
        </div>
      </div>
    `;
    ...
    const renderEmbeddingCacheList = async () => {
      ...
      if (!blocks.length) {
        wrap.innerHTML = `<div class="pse-assembly">目前沒有向量快取資料。</div>`;
        return;
      }
      wrap.innerHTML = blocks.map((b, idx) => `
        <div class="pse-entry-block" data-cache-card-key="${escapeHtml(b.cardKey)}" data-cache-parity="${idx % 2 === 0 ? "even" : "odd"}">
          <div class="pse-entry-grid" style="grid-template-columns:1fr auto;">
            <div>
              <div><b>卡片</b>：${escapeHtml(b.cardName)}</div>
              <div style="margin-top:4px;"><b>條目數</b>：${escapeHtml(String(b.entryCount))}</div>
              <div style="margin-top:4px;"><b>檔案大小</b>：${escapeHtml(formatBytes(b.sizeBytes))}</div>
            </div>
            <button class="pse-entry-remove" type="button" data-delete-cache-card="1">刪除</button>
          </div>
        </div>
      `).join("");
    };
    ...
    document.getElementById("pse-embed-cache-refresh")?.addEventListener("click", async () => {
      await renderEmbeddingCacheList();
      showStatus("已更新向量快取列表。", "info");
    });

    document.getElementById("pse-embed-cache-clear-all")?.addEventListener("click", async (e) => {
      if (!confirm("確定要清除全部向量快取嗎？\n這將刪除所有卡片的快取資料，下次發送時會重新建立。")) return;
      await clearAllEmbeddingCache();
      await renderEmbeddingCacheList();
      showStatus("已清除全部向量快取，下次發送時將重新建立。", "ok");
    });
    ...
    document.getElementById("pse-embed-cache-list")?.addEventListener("click", async (e) => {
      ...
      if (store.cards && store.cards[cardKey]) {
        ...
        embeddingCacheStore = null;
        // 明確傳入修改後的 store，避免 saveEmbeddingCacheStore 回頭讀記憶體快取把資料寫回去
        try { await Risuai.pluginStorage.removeItem(VCACHE_CARD_PREFIX + cardKey); } catch { }
        await saveEmbeddingCacheStore(store);
        // 清除 step0 狀態，使下次發送時強制重建
        try { await Risuai.pluginStorage.removeItem("static_knowledge_chunks"); } catch { }
        try { await Risuai.pluginStorage.removeItem("static_data_hash"); } catch { }
        try { await Risuai.pluginStorage.removeItem("step0_complete"); } catch { }
        embeddingVectorCache.clear();
      ...
      overlay.innerHTML = `
        <div class="pse-editor-modal">
          <div class="pse-editor-head">
            ...
            <button id="pse-editor-close" class="pse-editor-close" type="button" aria-label="關閉">✕</button>
          </div>
          <textarea id="pse-editor-textarea" class="pse-editor-textarea"></textarea>
          <div class="pse-editor-actions">
            <button id="pse-editor-cancel" class="pse-btn close" type="button" style="flex:0 0 auto">取消</button>
            <button id="pse-editor-apply" class="pse-btn save" type="button" style="flex:0 0 auto">套用</button>
          </div>
        </div>
      `;
    ...
    bindPromptExpander("advanced_model_anchor_prompt_expand", "advanced_model_anchor_prompt", "系統定位提示詞 - 放大編輯");
    bindPromptExpander("advanced_prefill_prompt_expand", "advanced_prefill_prompt", "助理預填充 - 放大編輯");
    bindPromptExpander("advanced_prereply_prompt_expand", "advanced_prereply_prompt", "預回覆提示詞 - 放大編輯");
    bindPromptExpander("init_bootstrap_model_anchor_prompt_expand", "init_bootstrap_model_anchor_prompt", "分類定位提示詞 - 放大編輯");
    ...
    document.querySelector(".pse-card").addEventListener("click", (e) => {
      ...
      if (textarea) {
        let title = "編輯提示詞內容";
        if (btn.classList.contains("pse-entry-format-expand")) title = "輸出格式 (JSON Schema) - 放大編輯";
        ...
        return `
          <div class="pse-entry-block" data-entry-index="${entryIndex}">
            <div class="pse-entry-grid">
              <div class="pse-entry-col"><label class="pse-label">Lorebook 條目</label><input class="pse-input pse-entry-lore" value="${escapeHtml(e.lorebook_name)}" /></div>
              <div class="pse-entry-col"><label class="pse-label">寫入模式 (覆蓋/添加)</label><select class="pse-input pse-entry-mode">${renderSelectOptions(LORE_WRITE_MODE_OPTIONS, e.write_mode)}</select></div>
              <div class="pse-entry-col"><label class="pse-label">始終啟用</label><select class="pse-input pse-entry-always-active"><option value="1" ${e.always_active ? "selected" : ""}>是</option><option value="0" ${!e.always_active ? "selected" : ""}>否</option></select></div>
            ...
            <div class="pse-entry-grid-row2">
              <div class="pse-entry-col">
                <label class="pse-label">輸出格式 (JSON Schema)</label>
                <div class="pse-textarea-wrap">
                  <textarea class="pse-entry-format-input pse-entry-format pse-textarea">${escapeHtml(String(e.output_format || ""))}</textarea>
                  <button class="pse-expand-btn pse-entry-format-expand" type="button" aria-label="放大編輯"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 4H5a1 1 0 0 0-1 1v3"/><path d="M16 4h3a1 1 0 0 1 1 1v3"/><path d="M20 16v3a1 1 0 0 1-1 1h-3"/><path d="M4 16v3a1 1 0 0 0 1 1h3"/></svg></button>
                </div>
              </div>
            </div>
            <div class="pse-entry-retention-row" style="display:${e.write_mode === "append" ? "flex" : "none"};align-items:center;gap:8px;flex-wrap:wrap;margin-top:4px;">
              <input class="pse-entry-retention-enabled" type="checkbox" id="ret_${callIndex}_${entryIndex}" ${e.retention_enabled ? "checked" : ""}
                title="啟用後，在達到指定回合數後，自動修剪此條目的舊資料，僅保留最新幾回合內容" />
              <label for="ret_${callIndex}_${entryIndex}" class="pse-label" style="cursor:pointer;user-select:none;">於</label>
              <input class="pse-input pse-entry-retention-after" type="number" min="0" value="${e.retention_after ?? 0}"
                style="width:60px;" title="累積超過此回合數後才開始修剪 (0 = 立即)" ${e.retention_enabled ? "" : "disabled"} />
              <label class="pse-label">回合後執行自動清理，僅保留最新的</label>
              <input class="pse-input pse-entry-retention-keep" type="number" min="0" value="${e.retention_keep ?? 5}"
                style="width:60px;" title="清理後保留最新幾回合的區塊 (0 = 清空全部)" ${e.retention_enabled ? "" : "disabled"} />
              <label class="pse-label">回合資料</label>
            </div>
          </div>
        `;
      }).join("");
          ...
          <div class="pse-call-card" data-call-index="${callIndex}" data-call-target="${escapeHtml(call.target_model)}" data-call-parity="${callIndex % 2 === 0 ? "even" : "odd"}">
            <div class="pse-call-head">
              <div class="pse-entry-col" style="display:flex;align-items:center;gap:6px;">
                <input class="pse-call-allow-parallel" type="checkbox" ${call.allow_parallel ? "checked" : ""} title="啟用後，此任務將與其他併發任務同時執行，縮短等待時間。" style="margin:0;flex-shrink:0;" />
                <label class="pse-label" style="margin:0;cursor:pointer;white-space:nowrap;">併發執行</label>
              </div>
              <div class="pse-entry-col"><label class="pse-label">呼叫備註</label><input class="pse-input pse-call-name" value="${escapeHtml(call.name)}" /></div>
              <div class="pse-entry-col">
                <label class="pse-label">模型</label>
                <select class="pse-input pse-call-target"><option value="A" ${call.target_model === "A" ? "selected" : ""}>主要模型</option><option value="B" ${call.target_model === "B" ? "selected" : ""}>輔助模型</option></select>
              </div>
              <div class="pse-entry-col"><label class="pse-label">觸發頻率 (回合)</label><input class="pse-input pse-call-frequency" type="number" min="1" value="${String(call.every_n_turns || 1)}" /></div>
            ...
            <div class="pse-call-row2">
              <div class="pse-entry-col"><label class="pse-label">讀取對話回合數 (0=全部)</label><input class="pse-input pse-call-read-rounds" type="number" min="0" value="${String(Math.max(0, toInt(call.read_dialogue_rounds, 4)))}" /></div>
              <div class="pse-entry-col"><label class="pse-label">關聯 Lorebook 條目 (逗號分隔)</label><input class="pse-input pse-call-read-lorebook-names" value="${escapeHtml(String(call.read_lorebook_names || ""))}" /></div>
            </div>
            <div class="pse-entry-list">${renderCallEntries(call, callIndex)}</div>
            <button class="pse-add-entry" type="button" data-add-entry="1" data-call-index="${callIndex}">新增寫入條目</button>
          </div>
        `;
      }).join("");
    };
    ...
    const readModelCallsFromUI = () => {
      ...
      return calls.length ? calls : [normalizeModelCall({ name: "目的備註A", target_model: "A", every_n_turns: 1, read_dialogue_rounds: 4, read_lorebook_names: "", entries: [defaultOutputEntry("A")] }, 0)];
    };
    ...
    document.getElementById("add_model_call")?.addEventListener("click", () => {
      ...
      uiModelCalls.push(normalizeModelCall({ id: `call_${Date.now()}_${nextIndex}`, name: `目的備註${nextIndex + 1}`, target_model: nextIndex % 2 === 0 ? "A" : "B", every_n_turns: 1, read_dialogue_rounds: 4, read_lorebook_names: "", entries: [defaultOutputEntry(nextIndex % 2 === 0 ? "A" : "B")] }, nextIndex));
      renderModelCalls();
    });
    ...
    modelCallListEl?.addEventListener("click", (e) => {
      ...
      if (removeCallBtn) {
        ...
        if (Number.isFinite(idx)) {
          ...
          if (uiModelCalls.length === 0) uiModelCalls = [normalizeModelCall({ name: "目的備註A", target_model: "A", every_n_turns: 1, read_dialogue_rounds: 4, read_lorebook_names: "", entries: [defaultOutputEntry("A")] }, 0)];
          renderModelCalls();
        }
        return;
      }
    ...
    // 切換 write_mode 時顯示/隱藏 retention 列；勾選 retention checkbox 時啟用/停用數值輸入
    ...
    document.getElementById("pse-reset-agent-defaults")?.addEventListener("click", async () => {
      try {
        ...
        showStatus("已重置為 Agent 預設。", "ok");
      } catch (e) { showStatus(`重置失敗：${e?.message || String(e)}`, "err"); }
    });
    ...
    const refreshEmbeddingPresetByProvider = async (applyPreset = true) => {
      ...
      // 切換供應商時強制套用 preset 的 format 和 url
    ...
    document.getElementById("pse-save")?.addEventListener("click", async () => {
      try {
        ...
        showStatus("設定已儲存。", "ok");
      } catch (e) { showStatus(`儲存失敗：${e?.message || String(e)}`, "err"); }
    });
  ...
  // ============================================================================
  // 9. 掛載點與 Replacer 註冊 (Hooks & Lifecycle)
  // ============================================================================
  ...
  async function ensureReplacerRegistered() {
    ...
    try {
      ...
      await Risuai.log(`${LOG} addRisuReplacer 失敗: ${err?.message || String(err)}`);
      return false;
    }
  }
  ...
  function _replacerBody(messages, type) {
    return (async (messages, type) => {
      ...
      // type="display"：主要模型已完成回覆，執行 retention 修剪後即返回
      if (type === "display") {
      ...
      if (needsStep0 && type === "model") {
        try {
          ...
          if (isVectorEnabled) {
            await Risuai.log(`${LOG} 啟動背景知識庫初始化 (Step 0)... 請稍候。`);
          } else {
            await Risuai.log(`${LOG} 啟動背景知識庫分類 (Step 0, 關鍵字模式)... 請稍候。`);
          }
          ...
          await Risuai.log(`${LOG} 知識庫初始化完成！`);
        } catch (step0Err) {
          const errMsg = step0Err?.message || String(step0Err);
          await Risuai.log(`${LOG} ❌ 知識庫初始化失敗中斷：${errMsg}`);
          if (isVectorEnabled) {
            throw new Error(`[RisuAI Agent] 向量知識庫建置失敗/超時。進度已保存。\n錯誤原因：${errMsg}\n請等待幾秒後，再次點擊「重新生成/發送」以繼續未完成的進度。`);
          } else {
            console.warn(`${LOG} Step0 分類失敗，使用 fallback (information)：${errMsg}`);
            await Risuai.log(`${LOG} ⚠️ 知識庫分類失敗，改以 fallback 模式繼續：${errMsg}`);
          }
        }
      }
      ...
      if (!usedCache) {
        ...
        if (dueCalls.length > 0) {
          await Risuai.log(`${LOG} Agent: 正在呼叫輔助模型進行分析與資料擷取...`);
          ...
          if (rejected.length > 0) {
            ...
            await abortMainModelWithAuxError(`輔助模型執行失敗：\n${errs}`);
          }
          ...
          for (const res of results) {
            if (res.status === "fulfilled") {
              ...
              if (raw) {
                ...
                } catch (e) { await abortMainModelWithAuxError(`條目儲存失敗：\n${e.message}`); }
              }
            }
          }
      ...
      if (!extractedData && !usedCache) {
        ...
        if (due.length > 0) await Risuai.log(`${LOG} 警告：輔助模型未回傳可用內容或格式無法解析。`);
      }
