//@name 👤 RisuAI Agent
//@display-name 👤 RisuAI Agent v1.3
...
6. Match key names and value types exactly.`,
  ...
  // ============================================================================
  // 3. Vector Database & Cache (pluginStorage Vector Cache)
  // Uses Risuai.pluginStorage instead of IndexedDB (no IDB access in iframe sandbox)
  // Uses an "index + per-card" split strategy:
  //   pse_vec_index      → { version, updatedAt, cardKeys: [...] }
  //   pse_vec_card:<key> → { cardKey, cardName, updatedAt, entries: {...} }
  // ============================================================================
  ...
  async function saveEmbeddingCacheStore(storeToSave = null) {
    ...
    try {
      ...
      } catch { /* cleanup failure does not affect main flow */ }
      // Write each card
  ...
  async function clearAllEmbeddingCache() {
    ...
    // 2. Clear pluginStorage vector index and all card data
    try {
      const idxRaw = await Risuai.pluginStorage.getItem(VCACHE_INDEX_KEY);
      if (idxRaw) {
        const idx = JSON.parse(idxRaw);
    ...
    // 3. Clear step0 state and static chunk data
  ...
  function makeCardCacheKey(charId, charName) {
    // Strip non-alphanumeric characters from ID to ensure clean format
  ...
  async function getActiveCardKey(char) {
    ...
    // Iterate over all possible ID fields to ensure uniqueness
  ...
  function parsePossiblyWrappedJson(text) {
    ...
    // 1. Preferentially extract all ```json ... ``` blocks and merge them
    ...
    // 2. Fallback hybrid strategy:
    //    First try lastBrace (correctly handles } inside string values),
    //    if parsing fails, fall back to depth-tracking (correctly handles multiple sibling JSON objects in model output).
  ...
  function resolveExtractorConfig() {
    ...
    // Read each provider's independently stored key from provider_key_map
  ...
  async function getCopilotBearerToken(rawGitHubToken) {
    ...
    const { res } = await fetchWithFallback(
      ...
      12000, "Copilot token refresh", true
    );
  ...
  // ============================================================================
  // 6. Text & Vector Algorithms
  // ============================================================================
  function getEmbeddingTokenLimit(requestModel) {
    // Per-input token limit; the API truncates inputs exceeding this length.
  ...
  function getEmbeddingBatchInputLimit(requestModel) {
    // Maximum number of inputs allowed per API request.
    const m = String(requestModel || "").toLowerCase();
  ...
  function getEmbeddingBatchTokenLimit(requestModel) {
    // Maximum total tokens allowed per API request (sum across all inputs).
    const m = String(requestModel || "").toLowerCase();
  ...
  function splitIntoParagraphChunks(content, maxChars = Infinity) {
    ...
    for (let level = 1; level <= 4; level++) {
      ...
      if (level === 1) {
        ...
        // L1 == 0 or == 1: do not descend further — entry is a single topic unit
        break;
      } else {
        // Level N>1: only split if parent level also had >= 2 sections
  ...
  async function fetchEmbeddingVectorsRemote(texts, cfg) {
    const runner = async () => {
      ...
      // Rough char-to-token ratio (conservative: 3 chars ≈ 1 token for CJK/mixed)
      const estimateTokens = (t) => Math.min(Math.ceil(String(t || "").length / 3), perInputTokenLimit);

      // Build batches that respect both the input-count limit and the total-token limit.
      const batches = [];
  ...
  async function getLorebookContextByVector(names, conversationMessages) {
    ...
    for (const entry of localLore) {
      const name = safeTrim(entry?.comment || "");
      // Only include entries whose name is in the wanted set
      if (!wanted.has(name)) continue;
      const content = String(entry?.content || "").trim();
  ...
  function normalizeOutputEntry(entry, target) {
    ...
    // retention_enabled: only valid when write_mode=append; when enabled, auto-trims old turn data
    // retention_after: pruning begins only after accumulating more than this many turns (0 = immediately)
    // retention_keep: number of most recent turn blocks to retain after each prune
    ...
    const retentionKeep = Math.max(0, toInt(entry?.retention_keep, 5));  // 0 = clear all
    return {
  ...
  function normalizeModelCall(call, index = 0) {
    ...
    const nameFallback = index === 0 ? "Call Note A" : index === 1 ? "Call Note B" : `Call Note ${index + 1}`;
    const every = Math.max(1, toInt(call?.every_n_turns, 1));
    const readRounds = Math.max(0, toInt(call?.read_dialogue_rounds, 4));  // 0 = read all
    const entries = parseOutputEntries(call?.entries, target);
    // allow_parallel: allows this call to fire concurrently with other allow_parallel calls (up to 3 in parallel)
        ...
        for (const { loreName, writeMode, alwaysActive, content } of writes) {
          ...
          if (writeMode === "overwrite") {
            ...
            if (existingIndex < 0) {
              ...
              // Turn-level dedup: skip if this exact Turn block already exists
  ...
  async function writeOutputsForCall(modelCall, raw, parsed, reqHash, roundIndex = 0) {
    ...
    if (entries.length > 1 && (!parsed || typeof parsed !== "object")) {
      // Multi-entry call must return a valid JSON object. Treat as a hard failure
      // so the caller can abort the main model rather than silently skip data.
  ...
  async function runStep0Classification(char, resolvedGlobalNote, staticDataHash, resumeMode = false) {
    ...
    lorebook.forEach((l, idx) => {
      ...
      // RisuAI uses alwaysActive internally, or the SillyTavern-compatible constant field to indicate "always active"
    ...
    if (resumeMode) {
      ...
      } catch { /* resumeMode read failed, fall through to full re-run */ }
    }
...
.pse-call-card[data-call-parity="odd"] { border-left-color:var(--pse-accent-green); background:linear-gradient(180deg, rgba(5, 150, 105, 0.12) 0%, var(--pse-input-bg) 100%); }
  ...
  async function abortMainModelWithAuxError(message) {
    const msg = String(message || "Auxiliary model call or processing failed");
    ...
    // Clear the cached hash so the next retry re-runs extraction instead of hitting stale cache
  ...
  async function renderSettingsUI() {
    ...
    document.body.innerHTML = `
      <div class="pse-body">
        <div class="pse-card">
          <h1 class="pse-title">👤 RisuAI Agent v1.3</h1>
          <div id="pse-status" class="pse-status"></div>
          ${renderModelDatalists()}

          <div class="pse-tabs">
            <button class="pse-tab" data-page="7">Help</button>
            <button class="pse-tab active" data-page="1">Model Settings</button>
            <button class="pse-tab" data-page="5">Vector Search</button>
          </div>
          <div class="pse-tabs pse-tabs-secondary">
            <button class="pse-tab" data-page="3">Global Prompt</button>
            <button class="pse-tab" data-page="2">Lorebook Calls</button>
            <button class="pse-tab" data-page="6">Cache Manager</button>
          </div>

          <div class="pse-page active" data-page="1">
            <div class="pse-section pse-model-section-a">
              <div class="pse-section-title">Main Model (Model A)</div>
              <label class="pse-label">Provider</label>
              <select id="extractor_a_provider" class="pse-input">${renderProviderOptions(configCache.extractor_a_provider)}</select>
              <label class="pse-label">Request Format</label>
              <select id="extractor_a_format" class="pse-input">${renderFormatOptions(configCache.extractor_a_format)}</select>
              <label class="pse-label">Request URL</label>
              <input id="extractor_a_url" class="pse-input" value="${String(configCache.extractor_a_url || "").replace(/"/g, "&quot;")}" />
              <label class="pse-label">API Key</label>
              <input id="extractor_a_key" class="pse-input" type="text" value="${String(configCache.extractor_a_key || "").replace(/"/g, "&quot;")}" />
              <label class="pse-label">Model</label>
              ...
              <label class="pse-label">Temperature</label>
              ...
              <label class="pse-label">Concurrency Mode</label>
              <select id="extractor_a_concurrency" class="pse-input">
                <option value="1" ${Number(configCache.extractor_a_concurrency) === 1 ? "selected" : ""}>Concurrent (allow multiple simultaneous requests)</option>
                <option value="0" ${Number(configCache.extractor_a_concurrency) === 0 ? "selected" : ""}>Sequential (single-threaded requests only)</option>
              </select>
            </div>

            <div class="pse-section pse-model-section-b">
              <div class="pse-section-title">Auxiliary Model (Model B)</div>
              <label class="pse-label">Provider</label>
              ...
              <select id="extractor_b_concurrency" class="pse-input">
                <option value="1" ${Number(configCache.extractor_b_concurrency) === 1 ? "selected" : ""}>Concurrent (allow multiple simultaneous requests)</option>
                <option value="0" ${Number(configCache.extractor_b_concurrency) === 0 ? "selected" : ""}>Sequential (single-threaded requests only)</option>
              </select>
            </div>
            ...
            <div class="pse-section pse-model-section-embed">
              <div class="pse-section-title">
                Embedding Model
                <span style="font-size:12px; color:var(--pse-accent-rose); font-weight:normal; display:block; margin-top:4px;">⚠️ Vectors from different embedding models are completely incompatible. If you change the model, clear the cache data below first.</span>
              </div>
              <label class="pse-label">Provider</label>
              ...
              <label class="pse-label" style="margin-top:10px;">Concurrency Mode</label>
              <select id="embedding_concurrency" class="pse-input">
                <option value="1" ${Number(configCache.embedding_concurrency) === 1 ? "selected" : ""}>Concurrent (allow multiple simultaneous requests)</option>
                <option value="0" ${Number(configCache.embedding_concurrency) === 0 ? "selected" : ""}>Sequential (single-threaded requests only)</option>
              </select>
            </div>
          </div>
          ...
          <div class="pse-page" data-page="2">
            <div class="pse-section">
              <div class="pse-section-title">Lorebook Write Call Settings</div>
              ...
              <button id="add_model_call" class="pse-add-entry" type="button">Add Call Task</button>
            </div>
          </div>

          <div class="pse-page" data-page="3">
            <div class="pse-section">
              <label class="pse-label">System Anchor Prompt</label>
              <div class="pse-textarea-wrap">
                ...
                <button id="advanced_model_anchor_prompt_expand" class="pse-expand-btn" type="button" aria-label="Expand Edit">
              ...
              <label class="pse-label">Assistant Prefill</label>
              <div class="pse-textarea-wrap">
                <textarea id="advanced_prefill_prompt" class="pse-textarea">${escapeHtml(configCache.advanced_prefill_prompt || "")}</textarea>
                <button id="advanced_prefill_prompt_expand" class="pse-expand-btn" type="button" aria-label="Expand Edit">
                  <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 4H5a1 1 0 0 0-1 1v3"/><path d="M16 4h3a1 1 0 0 1 1 1v3"/><path d="M20 16v3a1 1 0 0 1-1 1h-3"/><path d="M4 16v3a1 1 0 0 0 1 1h3"/></svg>
                </button>
              </div>
              <label class="pse-label">Assistant Pre-Reply Prompt</label>
              <div class="pse-textarea-wrap">
                <textarea id="advanced_prereply_prompt" class="pse-textarea">${escapeHtml(configCache.advanced_prereply_prompt || "")}</textarea>
                <button id="advanced_prereply_prompt_expand" class="pse-expand-btn" type="button" aria-label="Expand Edit">
                  <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 4H5a1 1 0 0 0-1 1v3"/><path d="M16 4h3a1 1 0 0 1 1 1v3"/><path d="M20 16v3a1 1 0 0 1-1 1h-3"/><path d="M4 16v3a1 1 0 0 0 1 1h3"/></svg>
                </button>
              </div>
            </div>
          ...
          <div class="pse-page" data-page="5">
            <div class="pse-section" style="border-left:4px solid var(--pse-accent-indigo); background:linear-gradient(180deg, rgba(79, 70, 229, 0.08) 0%, var(--pse-input-bg) 100%);">
              <div class="pse-section-title">Vector Search</div>
              <select id="vector_search_enabled" class="pse-input">
                <option value="1" ${Number(configCache.vector_search_enabled) === 1 ? "selected" : ""}>Enabled (select Lorebook entries by semantic relevance)</option>
                <option value="0" ${Number(configCache.vector_search_enabled) === 1 ? "" : "selected"}>Disabled (use traditional keyword matching)</option>
              </select>
              <label class="pse-label">Number of recent dialogue turns to use as search query</label>
              ...
              <label class="pse-label">Top K (number of entries to return)</label>
              ...
              <label class="pse-label">Minimum similarity score threshold (0~1)</label>
            ...
            <div class="pse-section" style="border-left:4px solid var(--pse-accent-cyan); background:linear-gradient(180deg, rgba(8, 145, 178, 0.08) 0%, var(--pse-input-bg) 100%);">
              <div class="pse-section-title">Classification Model Settings</div>
              <label class="pse-label">Use Model</label>
              <select id="init_bootstrap_target_model" class="pse-input">
                <option value="A" ${safeTrim(configCache.init_bootstrap_target_model) === "B" ? "" : "selected"}>Main Model (A)</option>
                <option value="B" ${safeTrim(configCache.init_bootstrap_target_model) === "B" ? "selected" : ""}>Auxiliary Model (B)</option>
              </select>
              <label class="pse-label">Classification Anchor Prompt</label>
              <div class="pse-textarea-wrap">
                ...
                <button id="init_bootstrap_model_anchor_prompt_expand" class="pse-expand-btn" type="button" aria-label="Expand Edit">
                  <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 4H5a1 1 0 0 0-1 1v3"/><path d="M16 4h3a1 1 0 0 1 1 1v3"/><path d="M20 16v3a1 1 0 0 1-1 1h-3"/><path d="M4 16v3a1 1 0 0 0 1 1h3"/></svg>
                </button>
              </div>
            </div>
          ...
          <div class="pse-page" data-page="6">
            <div class="pse-section">
              <div class="pse-section-title">Cache Manager</div>
              <div class="pse-row" style="margin-bottom:8px;">
                <button id="pse-embed-cache-refresh" class="pse-btn cache" type="button" style="flex:0 0 auto;">Refresh Cache List</button>
                <button id="pse-embed-cache-clear-all" class="pse-btn" type="button" style="flex:0 0 auto;background:#b44;">Clear All Caches</button>
              </div>
          ...
          <div class="pse-page" data-page="7">
            <div class="pse-section">
              <div class="pse-row" style="margin-bottom:8px;">
                <button id="pse-reset-agent-defaults" class="pse-btn cache" type="button" style="flex:0 0 auto;">Reset to Defaults</button>
              </div>
              <div class="pse-assembly">
                <b>▌ Core System: Narrative Auditing & State Extraction Engine</b><br/>
          ...
          <div class="pse-btn-row">
            <button id="pse-save" class="pse-btn save">Save Settings</button>
            <button id="pse-close" class="pse-btn close">Close</button>
          </div>
        </div>
      </div>
    `;
    ...
    const renderEmbeddingCacheList = async () => {
      ...
      if (!blocks.length) {
        wrap.innerHTML = `<div class="pse-assembly">No vector cache data available.</div>`;
        return;
      }
      wrap.innerHTML = blocks.map((b, idx) => `
        <div class="pse-entry-block" data-cache-card-key="${escapeHtml(b.cardKey)}" data-cache-parity="${idx % 2 === 0 ? "even" : "odd"}">
          <div class="pse-entry-grid" style="grid-template-columns:1fr auto;">
            <div>
              <div><b>Card</b>: ${escapeHtml(b.cardName)}</div>
              <div style="margin-top:4px;"><b>Entries</b>: ${escapeHtml(String(b.entryCount))}</div>
              <div style="margin-top:4px;"><b>File Size</b>: ${escapeHtml(formatBytes(b.sizeBytes))}</div>
            </div>
            <button class="pse-entry-remove" type="button" data-delete-cache-card="1">Delete</button>
          </div>
        </div>
      `).join("");
    };
    ...
    document.getElementById("pse-embed-cache-refresh")?.addEventListener("click", async () => {
      await renderEmbeddingCacheList();
      showStatus("Vector cache list refreshed.", "info");
    });

    document.getElementById("pse-embed-cache-clear-all")?.addEventListener("click", async (e) => {
      if (!confirm("Are you sure you want to clear all vector caches?\nThis will delete all card cache data. The cache will be rebuilt on the next send.")) return;
      await clearAllEmbeddingCache();
      await renderEmbeddingCacheList();
      showStatus("All vector caches cleared. Cache will be rebuilt on next send.", "ok");
    });
    ...
    document.getElementById("pse-embed-cache-list")?.addEventListener("click", async (e) => {
      ...
      if (store.cards && store.cards[cardKey]) {
        ...
        embeddingCacheStore = null;
        // Explicitly pass the modified store to avoid saveEmbeddingCacheStore reading the in-memory cache and writing the data back
        try { await Risuai.pluginStorage.removeItem(VCACHE_CARD_PREFIX + cardKey); } catch { }
        await saveEmbeddingCacheStore(store);
        // Clear step0 state to force rebuild on next send
        try { await Risuai.pluginStorage.removeItem("static_knowledge_chunks"); } catch { }
        try { await Risuai.pluginStorage.removeItem("static_data_hash"); } catch { }
        try { await Risuai.pluginStorage.removeItem("step0_complete"); } catch { }
        embeddingVectorCache.clear();
      ...
      await renderEmbeddingCacheList();
      showStatus("Card vector cache deleted.", "ok");
    });
      ...
      overlay.innerHTML = `
        <div class="pse-editor-modal">
          <div class="pse-editor-head">
            ...
            <button id="pse-editor-close" class="pse-editor-close" type="button" aria-label="Close">✕</button>
          </div>
          <textarea id="pse-editor-textarea" class="pse-editor-textarea"></textarea>
          <div class="pse-editor-actions">
            <button id="pse-editor-cancel" class="pse-btn close" type="button" style="flex:0 0 auto">Cancel</button>
            <button id="pse-editor-apply" class="pse-btn save" type="button" style="flex:0 0 auto">Apply</button>
          </div>
        </div>
      `;
    ...
    bindPromptExpander("advanced_model_anchor_prompt_expand", "advanced_model_anchor_prompt", "System Anchor Prompt - Expanded Edit");
    bindPromptExpander("advanced_prefill_prompt_expand", "advanced_prefill_prompt", "Assistant Prefill - Expanded Edit");
    bindPromptExpander("advanced_prereply_prompt_expand", "advanced_prereply_prompt", "Assistant Pre-Reply Prompt - Expanded Edit");
    bindPromptExpander("init_bootstrap_model_anchor_prompt_expand", "init_bootstrap_model_anchor_prompt", "Classification Anchor Prompt - Expanded Edit");
    ...
    document.querySelector(".pse-card").addEventListener("click", (e) => {
      ...
      if (textarea) {
        let title = "Edit Prompt Content";
        if (btn.classList.contains("pse-entry-format-expand")) title = "Output Format (JSON Schema) - Expanded Edit";
        ...
        return `
          <div class="pse-entry-block" data-entry-index="${entryIndex}">
            <div class="pse-entry-grid">
              <div class="pse-entry-col"><label class="pse-label">Lorebook Entry</label><input class="pse-input pse-entry-lore" value="${escapeHtml(e.lorebook_name)}" /></div>
              <div class="pse-entry-col"><label class="pse-label">Write Mode (Overwrite/Append)</label><select class="pse-input pse-entry-mode">${renderSelectOptions(LORE_WRITE_MODE_OPTIONS, e.write_mode)}</select></div>
              <div class="pse-entry-col"><label class="pse-label">Always Active</label><select class="pse-input pse-entry-always-active"><option value="1" ${e.always_active ? "selected" : ""}>Yes</option><option value="0" ${!e.always_active ? "selected" : ""}>No</option></select></div>
            ...
            <div class="pse-entry-grid-row2">
              <div class="pse-entry-col">
                <label class="pse-label">Output Format (JSON Schema)</label>
                <div class="pse-textarea-wrap">
                  <textarea class="pse-entry-format-input pse-entry-format pse-textarea">${escapeHtml(String(e.output_format || ""))}</textarea>
                  <button class="pse-expand-btn pse-entry-format-expand" type="button" aria-label="Expand Edit"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 4H5a1 1 0 0 0-1 1v3"/><path d="M16 4h3a1 1 0 0 1 1 1v3"/><path d="M20 16v3a1 1 0 0 1-1 1h-3"/><path d="M4 16v3a1 1 0 0 0 1 1h3"/></svg></button>
                </div>
              </div>
            </div>
            <div class="pse-entry-retention-row" style="display:${e.write_mode === "append" ? "flex" : "none"};align-items:center;gap:8px;flex-wrap:wrap;margin-top:4px;">
              <input class="pse-entry-retention-enabled" type="checkbox" id="ret_${callIndex}_${entryIndex}" ${e.retention_enabled ? "checked" : ""}
                title="When enabled, automatically trims old data from this entry after reaching the specified turn count, keeping only the most recent turns" />
              <label for="ret_${callIndex}_${entryIndex}" class="pse-label" style="cursor:pointer;user-select:none;">After</label>
              <input class="pse-input pse-entry-retention-after" type="number" min="0" value="${e.retention_after ?? 0}"
                style="width:60px;" title="Pruning starts after accumulating more than this many turns (0 = immediately)" ${e.retention_enabled ? "" : "disabled"} />
              <label class="pse-label">turns, auto-trim and keep only the latest</label>
              <input class="pse-input pse-entry-retention-keep" type="number" min="0" value="${e.retention_keep ?? 5}"
                style="width:60px;" title="Number of most recent turn blocks to keep after trimming (0 = clear all)" ${e.retention_enabled ? "" : "disabled"} />
              <label class="pse-label">turns of data</label>
            </div>
          </div>
        `;
      }).join("");
          ...
          <div class="pse-call-card" data-call-index="${callIndex}" data-call-target="${escapeHtml(call.target_model)}" data-call-parity="${callIndex % 2 === 0 ? "even" : "odd"}">
            <div class="pse-call-head">
              <div class="pse-entry-col" style="display:flex;align-items:center;gap:6px;">
                <input class="pse-call-allow-parallel" type="checkbox" ${call.allow_parallel ? "checked" : ""} title="When enabled, this task will execute concurrently with other parallel tasks to reduce wait time." style="margin:0;flex-shrink:0;" />
                <label class="pse-label" style="margin:0;cursor:pointer;white-space:nowrap;">Parallel Execution</label>
              </div>
              <div class="pse-entry-col"><label class="pse-label">Call Note</label><input class="pse-input pse-call-name" value="${escapeHtml(call.name)}" /></div>
              <div class="pse-entry-col">
                <label class="pse-label">Model</label>
                <select class="pse-input pse-call-target"><option value="A" ${call.target_model === "A" ? "selected" : ""}>Main Model</option><option value="B" ${call.target_model === "B" ? "selected" : ""}>Auxiliary Model</option></select>
              </div>
              <div class="pse-entry-col"><label class="pse-label">Trigger Frequency (turns)</label><input class="pse-input pse-call-frequency" type="number" min="1" value="${String(call.every_n_turns || 1)}" /></div>
            ...
            <div class="pse-call-row2">
              <div class="pse-entry-col"><label class="pse-label">Read Dialogue Turns (0=all)</label><input class="pse-input pse-call-read-rounds" type="number" min="0" value="${String(Math.max(0, toInt(call.read_dialogue_rounds, 4)))}" /></div>
              <div class="pse-entry-col"><label class="pse-label">Associated Lorebook Entries (comma-separated)</label><input class="pse-input pse-call-read-lorebook-names" value="${escapeHtml(String(call.read_lorebook_names || ""))}" /></div>
            </div>
            <div class="pse-entry-list">${renderCallEntries(call, callIndex)}</div>
            <button class="pse-add-entry" type="button" data-add-entry="1" data-call-index="${callIndex}">Add Write Entry</button>
          </div>
        `;
      }).join("");
    };
    ...
    const readModelCallsFromUI = () => {
      ...
      return calls.length ? calls : [normalizeModelCall({ name: "Call Note A", target_model: "A", every_n_turns: 1, read_dialogue_rounds: 4, read_lorebook_names: "", entries: [defaultOutputEntry("A")] }, 0)];
    };
    ...
    document.getElementById("add_model_call")?.addEventListener("click", () => {
      ...
      uiModelCalls.push(normalizeModelCall({ id: `call_${Date.now()}_${nextIndex}`, name: `Call Note ${nextIndex + 1}`, target_model: nextIndex % 2 === 0 ? "A" : "B", every_n_turns: 1, read_dialogue_rounds: 4, read_lorebook_names: "", entries: [defaultOutputEntry(nextIndex % 2 === 0 ? "A" : "B")] }, nextIndex));
      renderModelCalls();
    });
    ...
    modelCallListEl?.addEventListener("click", (e) => {
      ...
      if (removeCallBtn) {
        ...
        if (Number.isFinite(idx)) {
          ...
          if (uiModelCalls.length === 0) uiModelCalls = [normalizeModelCall({ name: "Call Note A", target_model: "A", every_n_turns: 1, read_dialogue_rounds: 4, read_lorebook_names: "", entries: [defaultOutputEntry("A")] }, 0)];
          renderModelCalls();
        }
        return;
      }
    ...
    // When write_mode changes: show/hide retention row; when retention checkbox is checked: enable/disable number inputs
    ...
    document.getElementById("pse-reset-agent-defaults")?.addEventListener("click", async () => {
      try {
        ...
        showStatus("Reset to Agent defaults.", "ok");
      } catch (e) { showStatus(`Reset failed: ${e?.message || String(e)}`, "err"); }
    });
    ...
    const refreshEmbeddingPresetByProvider = async (applyPreset = true) => {
      ...
      // After provider switch, automatically load that provider's previously saved key
      if (keyEl) {
      ...
      // Force apply preset's format and URL when provider changes
    ...
    document.getElementById("pse-save")?.addEventListener("click", async () => {
      ...
      } catch (e) { showStatus(`Save failed: ${e?.message || String(e)}`, "err"); }
    });
  ...
  function _replacerBody(messages, type) {
    return (async (messages, type) => {
      ...
      // type="display": main model has finished responding; apply retention pruning and return
      if (type === "display") {
      ...
      if (baseConversation.length === 0) {
        ...
        await Risuai.safeLocalStorage.setItem("last_lore_sync_error", "Skipped: no usable conversation text in beforeRequest payload.");
      ...
      if (!usedCache) {
        ...
        if (dueCalls.length > 0) {
          ...
          if (rejected.length > 0) {
            ...
            await abortMainModelWithAuxError(`Auxiliary model execution failed:\n${errs}`);
          }
          ...
          for (const res of results) {
            if (res.status === "fulfilled") {
              ...
              if (raw) {
                ...
                } catch (e) { await abortMainModelWithAuxError(`Entry save failed:\n${e.message}`); }
              }
            }
          }
  ...
  async function getConversationFromCurrentChat(limit) {
    ...
    for (const m of src) {
      ...
      if (!content) continue;
      // Skip the First Message: any assistant turn before the first user turn
